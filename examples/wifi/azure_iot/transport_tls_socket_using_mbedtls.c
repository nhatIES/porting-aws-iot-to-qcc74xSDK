                          ...
                          size_t xRootCaSize )
{
    ...
    if( lMbedtlsError != 0 )
    {
        printf( "Failed to parse server root CA certificate: lMbedtlsError[%d]= %s : %s.",
                                     ...
                                     size_t xClientCertSize )
{
    ...
    if( lMbedtlsError != 0 )
    {
        printf( "Failed to parse the client certificate: lMbedtlsError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ));
    }
                              ...
                              size_t xPrivateKeySize )
{
    ...
    if( lMbedtlsError != 0 )
    {
        printf( "Failed to parse the client key: lMbedtlsError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
    }
                                       ...
                                       const NetworkCredentials_t * pxNetworkCredentials )
{
    ...
    if( pxNetworkCredentials->ppcAlpnProtos != NULL )
    {
        ...
        if( lMbedtlsError != 0 )
        {
            printf( "Failed to configure ALPN protocol in mbed TLS: lMbedtlsError[%d]= %s : %s.",
                        lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                        mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
        }
    }
    ...
    if( pxNetworkCredentials->xDisableSni == pdFALSE )
    {
        ...
        if( lMbedtlsError != 0 )
        {
            printf( "Failed to set server name: lMbedtlsError[%d]= %s : %s.",
                        lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                        mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
        }
    }
    ...
    /* Set Maximum Fragment Length if enabled. */
        ...
        if( lMbedtlsError != 0 )
        {
            printf( "Failed to maximum fragment length extension: lMbedtlsError[%d]= %s : %s.",
                        lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                        mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
        }
                                      ...
                                      const NetworkCredentials_t * pxNetworkCredentials )
{
    ...
    if( lMbedtlsError != 0 )
    {
        printf( "Failed to set default SSL configuration: lMbedtlsError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
                                          ...
                                          const NetworkCredentials_t * pxNetworkCredentials )
{
    ...
    if( lMbedtlsError != 0 )
    {
        printf( "Failed to set up mbed TLS SSL context: lMbedtlsError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
    ...
    if( xRetVal == eTLSTransportSuccess )
    {
        ...
        if( lMbedtlsError != 0 )
        {
            printf( "Failed to perform TLS handshake: lMbedtlsError[%d]= %s : %s.",
                        lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                        mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
        ...
        {
            printf( "(Network connection %p) TLS handshake successful.",
                       pxNetworkContext );
        }
    }
                                         ...
                                         mbedtls_ctr_drbg_context * pxCtrDrgbContext )
{
    ...
    if( lMbedtlsError != 0 )
    {
        printf( "Failed to add entropy source: lMbedtlsError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
        xRetVal = eTLSTransportInternalError;
    }
    ...
    if( xRetVal == eTLSTransportSuccess )
    {
        ...
        if( lMbedtlsError != 0 )
        {
            printf( "Failed to seed PRNG: lMbedtlsError[%d]= %s : %s.",
                        lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                        mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
            xRetVal = eTLSTransportInternalError;
        }
    ...
    if( xRetVal == eTLSTransportSuccess )
    {
        printf( "Successfully initialized mbedTLS." );
    }
                                         ...
                                         uint32_t ulSendTimeoutMs )
{
    ...
    {
        printf( "Invalid input parameter(s): Arguments cannot be NULL. pxNetworkContext=%p, "
    ...
    {
        printf( "pucRootCa cannot be NULL." );
        xRetVal = eTLSTransportInvalidParameter;
    }
    else if( ( pxSSLContext = pvPortMalloc( sizeof( MbedSSLContext_t ) ) ) == NULL )
    {
        printf( "Failed to allocate mbed ssl context memmory ." );
        xRetVal = eTLSTransportInsufficientMemory;
    }
    else
    {
        ...
        {
            printf( "Failed to set receive timeout on socket %d.", xSocketStatus );
            xRetVal = eTLSTransportInternalError;
        }
        else if( ( xSocketStatus = Sockets_SetSockOpt( pxTlsTransportParams->xTCPSocket,
                                                       SOCKETS_SO_SNDTIMEO,
        ...
        {
            printf( "Failed to set send timeout on socket %d.", xSocketStatus );
            xRetVal = eTLSTransportInternalError;
        }
                                                    ...
                                                    usPort ) ) != 0 )
        {
            printf( "Failed to connect to %s with error %d.",
                        pcHostName,
                        xSocketStatus );
            xRetVal = eTLSTransportConnectFailure;
        }
        ...
        {
            printf( "Failed to initialize Mbedtls %d.", xRetVal );
        }
        else if( ( xRetVal = tlsSetup( pxNetworkContext, pcHostName,
                                       pxNetworkCredentials ) ) != eTLSTransportSuccess )
        {
            printf( "Failed to setup Mbedtls %d.", xRetVal );
        }
        else if( ( xRetVal = tlsHandshake( pxNetworkContext, pxNetworkCredentials ) ) != eTLSTransportSuccess )
        {
            printf( "Failed to do TLS handshake %d.", xRetVal );
        }
        else
        {
            printf( "(Network connection %p) Connection to %s established.",
                       pxNetworkContext,
                       pcHostName );
        }
...
void TLS_Socket_Disconnect( NetworkContext_t * pxNetworkContext )
{
    ...
    if( ( pxNetworkContext == NULL ) || ( pxNetworkContext->pParams != NULL ) )
    {
        ...
        printf( "(Network connection %p) TLS close-notify sent; received xxx as the TLS status can be ignored for close-notify.",
                 pxNetworkContext );
    }
        ...
        ( lMbedtlsError != MBEDTLS_ERR_SSL_WANT_WRITE ) )
    {
        if( lMbedtlsError == 0 )
        {
            printf( "(Network connection %p) TLS close-notify sent.",
                       pxNetworkContext );
        }
        else
        {
            printf( "(Network connection %p) Failed to send TLS close-notify: mbedTLSError[%d]= %s : %s.",
                         ...
                         size_t xBytesToRecv )
{
    ...
    if( ( lMbedtlsError == MBEDTLS_ERR_SSL_TIMEOUT ) ||
        ...
        ( lMbedtlsError == MBEDTLS_ERR_SSL_WANT_WRITE ) )
    {
        printf( "Failed to read data. However, a read can be retried on this error. "
    ...
    {
        printf( "Failed to read data: mbedTLSError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
    }
    else
                         ...
                         size_t xBytesToSend )
{
    ...
    if( ( lMbedtlsError == MBEDTLS_ERR_SSL_TIMEOUT ) ||
        ...
        ( lMbedtlsError == MBEDTLS_ERR_SSL_WANT_WRITE ) )
    {
        printf( "Failed to send data. However, send can be retried on this error. "
                    "mbedTLSError[%d]= %s : %s.", lMbedtlsError,
                    mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
    ...
    {
        printf( "Failed to send data:  mbedTLSError[%d]= %s : %s.",
                    lMbedtlsError, mbedtlsHighLevelCodeOrDefault( lMbedtlsError ),
                    mbedtlsLowLevelCodeOrDefault( lMbedtlsError ) );
    }
    else
