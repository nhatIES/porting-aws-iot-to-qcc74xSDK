...
static int proc_http_head(const uint8_t *buf, int buf_len, int *status_code, int *body_start_off)
{
    ...
    if (buf_len < 16) { // "HTTP/1.1 XYZ\r\n\r\n"
        return QCC74x_HTTPSC_RET_HTTP_ERR;
    }
    ...
    *status_code = atoi(status_code_buf);
    ...
    if ((head_end_pos = memmem(buf, buf_len, "\r\n\r\n", 4)) == NULL) {
        return QCC74x_HTTPSC_HEAD_END_NOT_FOUND;
    }
    *body_start_off = (uint8_t *)head_end_pos - buf + 4;

    return QCC74x_HTTPSC_OK;
}
...
int https_request(const char *server, uint16_t port, const uint8_t *request, int req_len, uint8_t *response, int *res_len)
{
    ...
    if (rcv_buf == NULL) {
        ...
        return QCC74x_HTTPSC_RET_ERR_MEM;
    }
    memset(rcv_buf, 0, sizeof(rcv_buf));

    uint32_t start_time = 0, stop_time = 0;
    start_time = xTaskGetTickCount();
    fd = qcc74x_TcpSslConnect(server, port);

    LOG_I("qcc74x connect fd = 0x%08lx\r\n", fd);

    if (fd == QCC74x_TCP_CREATE_CONNECT_ERR) {
        LOG_E("ssl connect error\r\n");
        ret_val = QCC74x_HTTPSC_RET_ERR;
        goto exit;
    } else {
        ret = qcc74x_TcpSslState(fd);
    }

    LOG_I("ret = %ld\r\n", ret);

    while (ret != QCC74x_TCP_NO_ERROR)
    {
        // count += 100;
        // vTaskDelay(100);
        if (ret == QCC74x_TCP_CONNECTING) {
            ret = qcc74x_TcpSslState(fd);
        } else if(ret == QCC74x_TCP_CONNECT_ERR) {
            LOG_E("ssl tcp connect failed\r\n");
            ret_val = QCC74x_HTTPSC_RET_ERR;
            goto exit;
        }
        if (QCC74x_HTTPS_RET_TIMEOUT < count) {
            LOG_E("ssl tcp connect timeout\r\n");
            ret_val = QCC74x_HTTPSC_RET_TIMEOUT;
            goto exit;
        }
    }
    ...
    if (ret == QCC74x_TCP_NO_ERROR) {
        send_ret = qcc74x_TcpSslSend(fd, request, req_len);
        if (send_ret > 0) {
            while(1) {
                rcv_ret = qcc74x_TcpSslRead(fd, rcv_buf, buf_sz);
                if (rcv_ret > 0) {
                    LOG_I("rcv_ret = %ld\r\n", rcv_ret);
                    if (!http_head_proced) {
                        ...
                        if (proc_head_r == QCC74x_HTTPSC_OK) {
                            write_len = rcv_ret - body_start_off;
                            if (write_len + resp_write_off > *res_len) {
                                ret_val = QCC74x_HTTPSC_RET_ERR_BUF_TOO_SMALL;
                                break;
                            }
                            http_head_proced = 1;
