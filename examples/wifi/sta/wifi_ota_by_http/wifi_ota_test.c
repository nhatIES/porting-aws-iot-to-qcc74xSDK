...
static err_t cb_altcp_recv_fn(void *arg, struct altcp_pcb *conn, struct pbuf *p, err_t err)
{
...
#if SHA256_SEL
    qcc74x_l1c_dcache_clean_range(recv_buffer, buffer_offset);
    ...
    status = qcc74x_flash_write((ota_addr + write_flash_offset), recv_buffer, buffer_offset);
    if (status != 0) {
...
static void wifi_ota_use_lwip_httpc(int argc, char **argv)
{
    ...
    status = qcc74x_flash_erase(ota_addr, bin_size); /* erase flash */
    if (status != 0) {
...
int cmd_wifi_ota_test(int argc, char **argv)
{
    ...
    /* default cmd like this: <wifi_ota_test 192.168.123.120 3365 /wifi_ota_qcc743.bin.ota\r\n> */
