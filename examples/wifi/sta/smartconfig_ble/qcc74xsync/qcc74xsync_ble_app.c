...
{
    qcc74x_ble_sync_t *p_index = (qcc74x_ble_sync_t *)p_arg;
    qcc74x_ble_sync_stop(p_index);
    vPortFree(p_index);
}
...
static void qcc74xsync_init(int err)
{
    if (err != 0) {
        return;
    }
    if (gp_index != NULL) {
        printf("qcc74xsync already started\r\n");
        return;
    }
#ifdef CONFIG_BT_STACK_CLI
    ble_cli_register();
#endif /* CONFIG_BT_STACK_CLI */
    gp_index = pvPortMalloc(sizeof(qcc74x_ble_sync_t));
    if (gp_index == NULL) {
        return;
    }
    qcc74x_ble_sync_start(gp_index,
...
void qcc74xsync_ble_start(void)
{
...
void qcc74xsync_ble_stop(void)
{
    qcc74x_ble_sync_stop(gp_index);
    vPortFree(gp_index);
    gp_index = NULL;
}
...
void qcc74xsync_scan_all_ap(void)
{
