...
static qcc74x_ble_sync_t *gp_index = NULL;
struct bt_conn *qcc74xsync_conn = NULL;
struct bt_gatt_exchange_params qcc74xsync_exchg_mtu;
...
static void qcc74xsync_exchange_func(struct bt_conn *conn, u8_t err,
			  struct bt_gatt_exchange_params *params)
{
	printf("Exchange %s\r\n", err == 0U ? "successful" : "failed");
}

static void qcc74xsync_disconnected(struct bt_conn *conn, u8_t reason)
{
	...
	qcc74xsync_conn = NULL;
}

static void qcc74xsync_connected(struct bt_conn *conn, u8_t err)
{
	...
	printf("%s\r\n",__func__);
	qcc74xsync_conn = conn;

	//exchange mtu size after connected.
	qcc74xsync_exchg_mtu.func = qcc74xsync_exchange_func;
	ret = bt_gatt_exchange_mtu(qcc74xsync_conn, &qcc74xsync_exchg_mtu);
	if (!ret) {
        ...
        switch (p_event->event_id) {
          ...
          case CMD_GET_VERSION:
              pro_trans_layer_ack_read(gp_index->pro_handle,
                                       QCC74xSYNC_BLE_VERSION,
                                       sizeof(QCC74xSYNC_BLE_VERSION));
              break;
#if defined(CONFIG_ZIGBEE_PROV)
          case CMD_ZB_SCAN:
...
static void __qcc74x_ble_sync_task (void *p_arg)
{
    uint16_t protocol_mtu = 0;
    struct queue_buf recv_buf;
    qcc74x_ble_sync_t *index = (qcc74x_ble_sync_t *)p_arg;
...
static struct bt_conn_cb qcc74xsync_conn_callbacks = {
	.connected	=   qcc74xsync_connected,
	.disconnected	=   qcc74xsync_disconnected,
};

int qcc74x_ble_sync_start(qcc74x_ble_sync_t *index,
                       ...
                       void *cb_arg)
{
    ...
    memset(index, 0, sizeof(qcc74x_ble_sync_t));
    gp_index = index;
    ...
    {
        isRegister = 1;
        bt_conn_cb_register(&qcc74xsync_conn_callbacks);
    }
    bt_gatt_service_register((struct bt_gatt_service *)&wifiprov_server);

    index->task_handle = xTaskCreateStatic(__qcc74x_ble_sync_task,
                                    (char*)"pro",
...
int qcc74x_ble_sync_stop(qcc74x_ble_sync_t *index)
{
    if (index == NULL) {
        return -1;
    }
...
#if defined(CONFIG_ZIGBEE_PROV)
int qcc74x_blezb_sync_start(qcc74x_ble_sync_t *index,
...
int qcc74x_blezb_sync_stop(qcc74x_ble_sync_t *index)
{
    if (index == NULL) {
		return -1;
	}
                ...
                struct net_buf_simple *buf)
{
	...
	if (!gp_index || gp_index->rbuf_len == 0) {
		printf("qcc74x_blemesh_sync_start not call or len is zero\n");
		return ;
	}
...
#if 1
	if(!gp_index){
		printf("qcc74x_blemesh_sync_start not call\n");
		return;
	}
	//gp_index->p_cur_conn = conn;
...
int qcc74x_blemesh_sync_start(qcc74x_ble_sync_t *index,
                       struct blesync_wifi_func *func,
                       pfn_complete_cb_t cb,
                       void *cb_arg)
{
	...
	index->task_handle = xTaskCreateStatic(__qcc74x_ble_sync_task, (char*)"pro",
							BLE_PROV_TASK_STACK_SIZE, index, 10,
							index->stack, &index->task);
	return 0;
}

int qcc74x_blemesh_sync_stop(qcc74x_ble_sync_t *index)
{
	if (index == NULL) {
		return -1;
	}
