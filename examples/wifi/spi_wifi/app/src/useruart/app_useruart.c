...
struct qcc74x_device_s *s_useruart = NULL;
...
{
    char ch;
    uint32_t intstatus = qcc74x_uart_get_intstatus(s_useruart);

    if (intstatus & UART_INTSTS_RX_FIFO) {
        while (qcc74x_uart_rxavailable(s_useruart)) {
            //uart_rxbuf[uart_rx_count++] = qcc74x_uart_getchar(s_useruart);
            ch = (char)qcc74x_uart_getchar(s_useruart);
    ...
    if (intstatus & UART_INTSTS_RTO) {
        qcc74x_uart_int_clear(s_useruart, UART_INTCLR_RTO);
        while (qcc74x_uart_rxavailable(s_useruart)) {
            //uart_rxbuf[uart_rx_count++] = qcc74x_uart_getchar(s_useruart);
            ch = (char)qcc74x_uart_getchar(s_useruart);
    ...
    if (intstatus & UART_INTSTS_TX_FIFO) {
        ...
        for (uint8_t i = 0; i < 27; i++) {
            //qcc74x_uart_putchar(s_useruart, uart_txbuf[i]);
        }
        qcc74x_uart_txint_mask(s_useruart, true);
    }
#endif
}
static void _uart_init(void)
{
    ...
    s_useruart = qcc74x_device_get_by_name("uart1");
    ...
    qcc74x_uart_init(s_useruart, &cfg);

    //qcc74x_uart_txint_mask(s_useruart, false);
    qcc74x_uart_rxint_mask(s_useruart, false);
    qcc74x_irq_attach(s_useruart->irq_num, _uart_isr, NULL);
    qcc74x_irq_enable(s_useruart->irq_num);
}
#if 1
