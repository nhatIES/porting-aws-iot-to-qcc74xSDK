...
#error "LUA GPIO LIB not support QCC74x_undef"
#elif defined(QCC743)
...
{
    uint32_t pin = (uint32_t)luaL_checkinteger(L, 2);
    bool value = qcc74x_gpio_read(gpio, pin);
...
static int gpio_uart_init(lua_State *L)
{
    ...
    qcc74x_gpio_uart_init(gpio, pin, uart_func);
    return 1;
}
