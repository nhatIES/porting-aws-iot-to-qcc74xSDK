...
static qcc74x_block_pool_t block_pool;
...
static void consumer_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (pdTRUE != xQueueReceive(queue, &block_ptr, portMAX_DELAY)) {
            ...
            if (qcc74x_block_pool_free(&block_pool, block_ptr)) {
...
static void producer_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (qcc74x_block_pool_alloc(&block_pool, &block_ptr, 100)) {
            LOG_W("alloc block pool faild\r\n");

            qcc74x_block_pool_info_get(&block_pool, &total, &free);
...
int main(void)
{
    ...
    _ASSERT_FUNC(0 == qcc74x_block_pool_create(&block_pool, BLK_SIZE, QCC74x_BLOCK_POOL_ALIGN_8, block_pool_memory, BLK_MEMORY_SIZE));
    qcc74x_block_pool_info_get(&block_pool, &block_count, NULL);
    ...
    _ASSERT_FUNC(0 == qcc74x_block_pool_add_mtx(&block_pool, block_pool_mtx_get, block_pool_mtx_put));
    _ASSERT_FUNC(0 == qcc74x_block_pool_add_sem(&block_pool, block_pool_sem_get, block_pool_sem_put));
