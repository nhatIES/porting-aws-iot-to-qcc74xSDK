...
static struct qcc74x_device_s *cam0_y;
static struct qcc74x_device_s *cam1_uv;
...
int main(void)
{
    ...
    cam0_y = qcc74x_device_get_by_name("cam0");
    cam1_uv = qcc74x_device_get_by_name("cam1");
    ...
    qcc74x_cam_init(cam0_y, &cam_config);
    ...
    qcc74x_cam_init(cam1_uv, &cam_config);

    qcc74x_cam_start(cam0_y);
    qcc74x_cam_start(cam1_uv);

    for (i = 0; i < CAM_FRAME_COUNT_USE; i++) {
        while (qcc74x_cam_get_frame_count(cam0_y) == 0 || qcc74x_cam_get_frame_count(cam1_uv) == 0) {
        }
        pic_size_y = qcc74x_cam_get_frame_info(cam0_y, &pic_y);
        pic_size_uv = qcc74x_cam_get_frame_info(cam1_uv, &pic_uv);
        qcc74x_cam_pop_one_frame(cam0_y);
        qcc74x_cam_pop_one_frame(cam1_uv);
    ...
    qcc74x_cam_stop(cam0_y);
    qcc74x_cam_stop(cam1_uv);
