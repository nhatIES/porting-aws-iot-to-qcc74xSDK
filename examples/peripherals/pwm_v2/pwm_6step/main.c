...
{
    uint8_t hall = 0;
    if (qcc74x_gpio_read(gpio, GPIO_PIN_32)) {
        hall |= (1 << 2);
    }
    if (qcc74x_gpio_read(gpio, GPIO_PIN_33)) {
        hall |= (1 << 1);
    }
    if (qcc74x_gpio_read(gpio, GPIO_PIN_34)) {
        hall |= (1 << 0);
    }
    return hall;
}
...
void pwm_isr(int irq, void *arg)
{
    ...
    *(volatile uint32_t *)(GLB_BASE + GLB_GPIO_CFG17_OFFSET) = 0x00400B6A | (1 << 24);
    ...
    if (hall.angle == ANGLE0) {
        qcc74x_pwm_v2_channel_positive_stop(pwm, PWM_TRI_CH_U);
        qcc74x_pwm_v2_channel_positive_start(pwm, PWM_TRI_CH_V);
        qcc74x_gpio_set(gpio, PWM_PIN_W_L);
    } else if (hall.angle == ANGLE60) {
        qcc74x_gpio_reset(gpio, PWM_PIN_W_L);
        qcc74x_pwm_v2_channel_positive_start(pwm, PWM_TRI_CH_V);
        qcc74x_gpio_set(gpio, PWM_PIN_U_L);
    } else if (hall.angle == ANGLE120) {
        qcc74x_pwm_v2_channel_positive_stop(pwm, PWM_TRI_CH_V);
        qcc74x_pwm_v2_channel_positive_start(pwm, PWM_TRI_CH_W);
        qcc74x_gpio_set(gpio, PWM_PIN_U_L);
    } else if (hall.angle == ANGLE180) {
        qcc74x_gpio_reset(gpio, PWM_PIN_U_L);
        qcc74x_pwm_v2_channel_positive_start(pwm, PWM_TRI_CH_W);
        qcc74x_gpio_set(gpio, PWM_PIN_V_L);
    } else if (hall.angle == ANGLE240) {
        qcc74x_pwm_v2_channel_positive_stop(pwm, PWM_TRI_CH_W);
        qcc74x_pwm_v2_channel_positive_start(pwm, PWM_TRI_CH_U);
        qcc74x_gpio_set(gpio, PWM_PIN_V_L);
    } else if (hall.angle == ANGLE300) {
        qcc74x_gpio_reset(gpio, PWM_PIN_V_L);
        qcc74x_pwm_v2_channel_positive_start(pwm, PWM_TRI_CH_U);
        qcc74x_gpio_set(gpio, PWM_PIN_W_L);
    }
...
void peri_pwm_init(void)
{
    ...
    qcc74x_gpio_init(gpio, PWM_PIN_U_L, GPIO_OUTPUT | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_0);
    qcc74x_gpio_init(gpio, PWM_PIN_V_L, GPIO_OUTPUT | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_0);
    qcc74x_gpio_init(gpio, PWM_PIN_W_L, GPIO_OUTPUT | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_0);
    ...
    qcc74x_pwm_v2_channel_set_threshold(pwm, PWM_TRI_CH_U, 0, 1000);
    qcc74x_pwm_v2_channel_set_threshold(pwm, PWM_TRI_CH_V, 0, 1000);
    qcc74x_pwm_v2_channel_set_threshold(pwm, PWM_TRI_CH_W, 0, 1000);
    qcc74x_pwm_v2_channel_positive_stop(pwm, PWM_TRI_CH_U);
    qcc74x_pwm_v2_channel_positive_stop(pwm, PWM_TRI_CH_V);
    qcc74x_pwm_v2_channel_positive_stop(pwm, PWM_TRI_CH_W);
    /* config interrupt */
...
int main(void)
{
    ...
    qcc74x_gpio_init(gpio, GPIO_PIN_32, GPIO_INPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_0);
    qcc74x_gpio_init(gpio, GPIO_PIN_33, GPIO_INPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_0);
    qcc74x_gpio_init(gpio, GPIO_PIN_34, GPIO_INPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_0);
    peri_pwm_init();
