...
int main(void)
{
    ...
    qcc74x_aes_setkey(aes, aes_ctr_128bit_key, 128);
    qcc74x_aes_encrypt(aes, aes_ctr_pt_buffer, aes_ctr_128bit_iv, aes_ctr_ct_buffer, AES_DATA_LEN);
    qcc74x_aes_decrypt(aes, aes_ctr_ct_buffer, aes_ctr_128bit_iv, aes_ctr_tmp_buffer, AES_DATA_LEN);
    qcc74x_data_compare(aes_ctr_pt_buffer, aes_ctr_tmp_buffer, AES_DATA_LEN);
    ...
    qcc74x_flash_erase(0x00010000, 4096);

    memset(aes_ctr_tmp_buffer, 0, AES_DATA_LEN);
    /* write 0x00010000 flash data */
    qcc74x_flash_write(0x00010000, aes_ctr_ct_buffer, AES_DATA_LEN);
    qcc74x_flash_read(0x00010000, aes_ctr_tmp_buffer, AES_DATA_LEN);
    qcc74x_data_compare(aes_ctr_ct_buffer, aes_ctr_tmp_buffer, AES_DATA_LEN);
    printf("flash write & read success\r\n");

    struct qcc74x_flash_aes_config_s config;
    config.region = 0;
    ...
    qcc74x_flash_aes_init(&config);
    qcc74x_flash_aes_enable();

    /* read 0x00010000 flash data */
    qcc74x_data_compare(aes_ctr_pt_buffer, (uint8_t *)(FLASH_XIP_BASE - qcc74x_flash_get_image_offset() + 0x00010000), AES_DATA_LEN);
    printf("flash decrypt with flash aes ctr128 success\r\n");

    qcc74x_flash_read(0x00010000, aes_ctr_tmp_buffer, AES_DATA_LEN);
    qcc74x_aes_decrypt(aes, &aes_ctr_tmp_buffer[0], aes_ctr_128bit_iv, &aes_ctr_tmp_buffer2[0], 1024);  /* set new iv */
    qcc74x_aes_decrypt(aes, &aes_ctr_tmp_buffer[1024], NULL, &aes_ctr_tmp_buffer2[1024], 1024);         /* use last iv */
    qcc74x_aes_decrypt(aes, &aes_ctr_tmp_buffer[1024 * 2], NULL, &aes_ctr_tmp_buffer2[1024 * 2], 1024); /* use last iv */
    qcc74x_aes_decrypt(aes, &aes_ctr_tmp_buffer[1024 * 3], NULL, &aes_ctr_tmp_buffer2[1024 * 3], 1024); /* use last iv */
