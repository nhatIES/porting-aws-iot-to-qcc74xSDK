...
{
    printf("cost time:%d us\r\n", (uint32_t)(qcc74x_mtimer_get_time_us() - start_time));
    dma_tc_flag0++;
    printf("tc done\r\n");
}
...
int main(void)
{
    ...
    /* erase flash */
    qcc74x_flash_erase(FLASH_RW_START_ADDR, 4096);

    /* write flash data */
    qcc74x_flash_write(FLASH_RW_START_ADDR, write_buf, sizeof(write_buf)); /* FLASH_XIP_BASE - 0x2000 + 0x00010000 */
    ...
    for (uint8_t i = 0; i < sizeof(src_burst); i++) {
        ...
        transfers[0].src_addr = (uint32_t)(FLASH_XIP_BASE - qcc74x_flash_get_image_offset() + FLASH_RW_START_ADDR + DMA_FLASH_ADDR_OFFSET);
