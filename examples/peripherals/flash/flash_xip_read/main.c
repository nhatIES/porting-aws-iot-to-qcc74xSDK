...
int main(void)
{
    ...
    for (uint32_t offset = 0; offset < FLASH_RW_TOTAL_SIZE; offset+=sizeof(write_buf)) {
        ...
        /* erase flash */
        qcc74x_flash_erase(flash_addr , sizeof(write_buf));

        /* read flash data */
        read_buf = (uint8_t *)(FLASH_XIP_BASE + flash_addr - qcc74x_flash_get_image_offset());
        qcc74x_l1c_dcache_invalidate_range(read_buf, sizeof(write_buf));
        ...
        /* write flash data */
        qcc74x_flash_write(flash_addr, write_buf, sizeof(write_buf));
