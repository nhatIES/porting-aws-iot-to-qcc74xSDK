...
int main(void)
{
    ...
    for (uint8_t iomode = FLASH_IOMODE_NIO; iomode <= FLASH_IOMODE_QIO; iomode++) {
        ...
        qcc74x_flash_set_iomode(iomode);

        start_time = qcc74x_mtimer_get_time_ms();

        /* erase  flash */
        qcc74x_flash_erase(FLASH_RW_START_ADDR, sizeof(write_buf));

        memset(read_buf, 0, sizeof(read_buf));
        /* read flash data */
        qcc74x_flash_read(FLASH_RW_START_ADDR, read_buf, sizeof(read_buf));
        ...
        /* write flash data */
        qcc74x_flash_write(FLASH_RW_START_ADDR, write_buf, sizeof(write_buf));

        memset(read_buf, 0, sizeof(read_buf));
        /* read flash data */
        qcc74x_flash_read(FLASH_RW_START_ADDR, read_buf, sizeof(read_buf));

        printf("cost time:%lld ms\r\n", (qcc74x_mtimer_get_time_ms() - start_time));
