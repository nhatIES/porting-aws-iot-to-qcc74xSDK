...
static ATTR_NOCACHE_RAM_SECTION struct qcc74x_dma_channel_lli_pool_s llipool[1];
static ATTR_NOCACHE_RAM_SECTION struct qcc74x_dma_channel_lli_transfer_s transfers[1];
...
void rgb_to_buff(uint32_t *rgb, uint16_t *buff, uint32_t len, uint8_t pin)
{
    ...
    qcc74x_l1c_dcache_clean_invalidate_range(buff, sizeof(uint16_t) * BUFF_LEN);
}
...
int main(void)
{
    ...
    qcc74x_wo_pin_init(wo, PIN_USE, WO_MODE_WRITE);
    qcc74x_wo_enable_dma(wo);
    dma0_ch0 = qcc74x_device_get_by_name("dma0_ch0");
    qcc74x_dma_channel_tcint_mask(dma0_ch0, 1);
    ...
    while (1) {
        ...
        // qcc74x_wo_push_fifo(buff, BUFF_LEN);
        qcc74x_dma_channel_init(dma0_ch0, &dma_cfg);
        qcc74x_dma_channel_lli_reload(dma0_ch0, llipool, 1, transfers, 1);
