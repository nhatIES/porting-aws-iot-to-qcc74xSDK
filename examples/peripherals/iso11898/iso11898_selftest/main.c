...
struct qcc74x_iso11898_config_s cfg = {
    .prescaler = 19,                  /*!< specifies the length of a time quantum, ranges from 1 to 63, tq=tclk*2*(prescaler+1) */
...
struct qcc74x_iso11898_msg_s msg_rx;
...
int main(void)
{
    ...
    ret = qcc74x_iso11898_send(iso11898, &msg_tx, 1000);
    if (ret != 0) {
        printf("send failed!\r\n");
        while (1) {
        }
    ...
    ret = qcc74x_iso11898_recv(iso11898, &msg_rx, 10000);
    if (ret != 0) {
        printf("recv failed!\r\n");
        while (1) {
        }
