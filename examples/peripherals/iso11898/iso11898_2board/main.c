#define DEVICE_TX 1
#define PRESCALER 1
#define BS1       ISO11898_BS1_5TQ
#define BS2       ISO11898_BS2_4TQ
...
struct qcc74x_iso11898_config_s cfg = {
    .prescaler = PRESCALER,           /*!< specifies the length of a time quantum, ranges from 1 to 63, tq=tclk*2*(prescaler+1) */
    .mode = ISO11898_MODE_NORMAL,     /*!< specifies the CAN operating mode. @ref ISO11898_MODE_XXX */
    .sample = ISO11898_SAMPLE_SINGLE, /*!< specifies the number of sample times. @ref ISO11898_SAMPLE_XXX */
    ...
    .bs1 = BS1,                       /*!< specifies the number of time quanta in Bit Segment 1. @ref ISO11898_BS1_XXX */
    .bs2 = BS2,                       /*!< specifies the number of time quanta in Bit Segment 2. @ref ISO11898_BS2_XXX */
};
...
void iso11898_isr(int irq, void *arg)
{
    ...
    if (int_status & ISO11898_INTSTS_RX) {
        ...
        if (msg.rtr == ISO11898_RTR_DATA) {
            ...
            // printf("\r\n");
            if ((msg.dlc == 8) && \
                (msg.data[0] == msg_tx.data[0]) && \
                (msg.data[1] == msg_tx.data[1]) && \
                (msg.data[2] == msg_tx.data[2]) && \
                (msg.data[3] == msg_tx.data[3]) && \
                (msg.data[4] == msg_tx.data[4]) && \
                (msg.data[5] == msg_tx.data[5]) && \
                (msg.data[6] == msg_tx.data[6]) && \
                (msg.data[7] == msg_tx.data[7])) {
                    static uint32_t cnt = 0;
                    msg.data[0] = 0;
                    msg.data[1] = 0;
                    msg.data[2] = 0;
