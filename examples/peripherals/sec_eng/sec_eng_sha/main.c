...
int main(void)
{
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            ...
            for (uint32_t j = 0; j < 1000; j++) {
                qcc74x_sha1_update(sha, &ctx_sha1, sha_input_buf, sha_test_buflen[i]);
            }
        } else {
        ...
        qcc74x_data_compare(sha1_test_sum[i], sha_output_buf, 20);
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            memset(sha_input_buf, 'a', 1000);
            for (uint32_t j = 0; j < 1000; j++) {
                qcc74x_sha256_update(sha, &ctx_sha256, sha_input_buf, sha_test_buflen[i]);
            }
        } else {
            memcpy(sha_input_buf, sha_test_buf[i], sha_test_buflen[i]);
            qcc74x_sha256_update(sha, &ctx_sha256, sha_input_buf, sha_test_buflen[i]);
        ...
        qcc74x_data_compare(sha256_test_sum[i], sha_output_buf, 28);
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        qcc74x_sha256_finish(sha, &ctx_sha256, sha_output_buf);
        qcc74x_data_compare(sha256_test_sum[i + 3], sha_output_buf, 32);
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            memset(sha_input_buf, 'a', 1000);
            for (uint32_t j = 0; j < 1000; j++) {
                qcc74x_sha512_update(sha, &ctx_sha512, sha_input_buf, sha512_test_buflen[i]);
            }
        } else {
        ...
        qcc74x_data_compare(sha512_test_sum[i], sha_output_buf, 48);
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        qcc74x_sha512_finish(sha, &ctx_sha512, sha_output_buf);
        qcc74x_data_compare(sha512_test_sum[i + 3], sha_output_buf, 64);
    }
