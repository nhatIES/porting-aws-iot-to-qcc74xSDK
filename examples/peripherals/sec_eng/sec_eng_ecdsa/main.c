...
int main(void)
{
    ...
    /* We use following test vector to show how QCC74x_ECDSA works��
...
#if 0
    time = (unsigned int)qcc74x_mtimer_get_time_ms();
    qcc74x_sec_ecdsa_get_public_key(&ecdsa_handle, (uint32_t *)ecdsa_private_key, tmp_keyx, tmp_keyy);
    printf("Get public key time=%dms\r\n", (unsigned int)qcc74x_mtimer_get_time_ms() - time);
    ...
    if (0 != qcc74x_sec_ecdsa_sign(&ecdsa_handle, (uint32_t *)ecdsa_k, (uint32_t *)ecdsa_hash, sizeof(ecdsa_hash) / 4, tmp_keyx, tmp_keyy)) {
        printf("Sign Fail\r\n");
    }
    ...
    if (0 != qcc74x_sec_ecdsa_verify(&ecdsa_handle, (uint32_t *)ecdsa_hash, sizeof(ecdsa_hash) / 4, tmp_keyx, tmp_keyy)) {
        printf("Verify Fail\r\n");
    } else {
