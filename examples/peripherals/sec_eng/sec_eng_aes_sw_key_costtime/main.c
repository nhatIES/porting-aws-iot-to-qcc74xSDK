...
int main(void)
{
    ...
    for (uint32_t i = 1; i <= 1024; i++) {
        ...
        qcc74x_aes_set_mode(aes, AES_MODE_XTS);
        qcc74x_aes_setkey(aes, aes_256bit_key, 128);
        memset(aes_iv, 0, 16);
        start_time = qcc74x_mtimer_get_time_us();
        qcc74x_aes_encrypt(aes, aes_input_buf, aes_iv, aes_enc_buf, 16 * i);
