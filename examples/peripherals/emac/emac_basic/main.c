...
void emac_irq_cb(void *arg, uint32_t irq_event, struct qcc74x_emac_trans_desc_s *trans_desc)
{
    switch (irq_event) {
...
int emac_test_init(void)
{
    ...
    /* phy cfg */
    eth_phy_init_cfg_t phy_cfg = {
        .speed_mode = EPHY_SPEED_MODE_100M_FULL_DUPLEX,
    };
#endif
    ...
    qcc74x_emac_irq_attach(emac0, emac_irq_cb, NULL);
...
void emac_test(void)
{
    ...
    struct qcc74x_emac_trans_desc_s tx_test_desc = {
        .buff_addr = eth_tx_buff,
        .data_len = sizeof(arp_data),
    };

    struct qcc74x_emac_trans_desc_s rx_test_desc = {
    ...
    time_node = qcc74x_mtimer_get_time_ms();

    while (1) {
        /* try to push tx */
        if (qcc74x_emac_feature_control(emac0, EMAC_CMD_GET_TX_DB_AVAILABLE, 0) > 0) {
            if (qcc74x_emac_queue_tx_push(emac0, &tx_test_desc) == 0) {
                tx_push_cnt += 1;
            }
        }

        /* try to push rx */
        if (qcc74x_emac_feature_control(emac0, EMAC_CMD_GET_RX_DB_AVAILABLE, 0) > 0) {
            if (qcc74x_emac_queue_rx_push(emac0, &rx_test_desc) == 0) {
                rx_push_cnt += 1;
            }
        }

        /* get info */
        if (qcc74x_mtimer_get_time_ms() - time_node > 2 * 1000) {
            time_node = qcc74x_mtimer_get_time_ms();
