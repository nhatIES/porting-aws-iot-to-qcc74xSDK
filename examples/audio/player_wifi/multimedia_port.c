...
static struct qcc74x_device_s *i2s0;
...
#endif
    //qcc74x_i2s_txint_mask(i2s0, 0);
    //qcc74x_i2s_rxint_mask(i2s0, 0);
        //qcc74x_i2s_feature_control(i2s0, I2S_CMD_DATA_ENABLE, I2S_CMD_DATA_ENABLE_TX);
}
...
{
    if (i2s0) {
        qcc74x_i2s_feature_control(i2s0, I2S_CMD_DATA_ENABLE, I2S_CMD_DATA_ENABLE_RX);
    }
}
#endif
...
//#if CONFIG_ENABLE_BOTTOM_PA
    ...
    //qcc74x_gpio_enable_output(pa_pin, 1, 0);
...
// #if CONFIG_ENABLE_BOTTOM_PA
//     // init
//     qcc74x_gpio_enable_output(CONFIG_AUDIO_PA_PIN, 1, 0);
// #endif
//#if CONFIG_ENABLE_BOTTOM_PA
    // gpio set 0 ---> disable
    //msp_msleep(1000);
...
extern void qcc74x_irq_unregister(int irqnum, void *handler);
extern void qcc74x_irq_enable(unsigned int source);
extern void qcc74x_irq_disable(unsigned int source);

// irq register
void msp_irq_register(int irqnum, void *handler, void *ctx)
{
   qcc74x_irq_register_with_ctx(irqnum, handler, ctx);
}

void msp_irq_unregister(int irqnum)
{
   qcc74x_irq_register_with_ctx(irqnum, NULL, NULL);
}

void msp_irq_enable(int irqnum)
{
   qcc74x_irq_enable((unsigned int)irqnum);
}

void msp_irq_disenable(int irqnum)
{
   qcc74x_irq_disable((unsigned int)irqnum);
}
