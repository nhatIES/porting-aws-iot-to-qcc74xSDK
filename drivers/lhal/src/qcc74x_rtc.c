...
{
#ifdef romapi_qcc74x_rtc_disable
    romapi_qcc74x_rtc_disable(dev);
#else
    uint32_t reg_base;
    uint32_t regval;

    reg_base = QCC74x_RTC_BASE;
...
{
#ifdef romapi_qcc74x_rtc_set_time
    romapi_qcc74x_rtc_set_time(dev, time);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint64_t rtc_cnt;
...
{
#ifdef romapi_qcc74x_rtc_get_time
    return romapi_qcc74x_rtc_get_time(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint64_t time_l;
...
uint64_t qcc74x_rtc_get_delta_counter(uint64_t ref_cnt)
{
    ...
    cnt = qcc74x_rtc_get_time(NULL);
    ref_cnt &= QCC74x_RTC_MAX_COUNTER;

    if (cnt < ref_cnt) {
        cnt += QCC74x_RTC_MAX_COUNTER + 1;
    }
...
uint64_t qcc74x_rtc_get_delta_time_ms(uint64_t ref_cnt)
{
    ...
    return QCC74x_RTC_COUNTER_TO_MS(cnt);
}
