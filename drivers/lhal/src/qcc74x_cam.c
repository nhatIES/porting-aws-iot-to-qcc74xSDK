...
#ifdef romapi_qcc74x_cam_init
    romapi_qcc74x_cam_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint8_t data_mode = 0;
...
#endif
        regval = qcc74x_clk_get_system_clock(QCC74x_SYSTEM_PBCLK) / 1000000;
        if (regval == 0) {
            regval = 80;
        }
...
{
#ifdef romapi_qcc74x_cam_start
    romapi_qcc74x_cam_start(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_cam_stop
    romapi_qcc74x_cam_stop(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_cam_int_mask(struct qcc74x_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qcc74x_cam_int_mask
    romapi_qcc74x_cam_int_mask(dev, int_type, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_cam_int_clear
    romapi_qcc74x_cam_int_clear(dev, int_type);
#else
...
{
#ifdef romapi_qcc74x_cam_crop_vsync
    romapi_qcc74x_cam_crop_vsync(dev, start_line, end_line);
#else
...
{
#ifdef romapi_qcc74x_cam_crop_hsync
    romapi_qcc74x_cam_crop_hsync(dev, start_pixel, end_pixel);
#else
#if defined(QCC74x_undef)
...
{
#ifdef romapi_qcc74x_cam_pop_one_frame
    romapi_qcc74x_cam_pop_one_frame(dev);
#else
#if defined(QCC74x_undef)
...
{
#ifdef romapi_qcc74x_cam_get_frame_count
    return romapi_qcc74x_cam_get_frame_count(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_cam_get_frame_info
    return romapi_qcc74x_cam_get_frame_info(dev, pic);
#else
    uint32_t reg_base;
...
{
#ifdef romapi_qcc74x_cam_get_intstatus
    return romapi_qcc74x_cam_get_intstatus(dev);
#else
...
{
#ifdef romapi_qcc74x_cam_feature_control
    return romapi_qcc74x_cam_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
