 ...
 *
*******************************************************************************/
static void ATTR_TCM_SECTION qcc74x_ef_ctrl_switch_ahb_clk_r0(struct qcc74x_device_s *dev)
{
    uint32_t reg_val;
...
static void ATTR_TCM_SECTION qcc74x_ef_ctrl_switch_ahb_clk_r1(struct qcc74x_device_s *dev)
{
    uint32_t reg_val;
    uint32_t timeout = EF_CTRL_DFT_TIMEOUT_VAL;
...
*******************************************************************************/
#ifdef EF_CTRL_EFUSE_R1_SIZE
static void ATTR_TCM_SECTION qcc74x_ef_ctrl_program_efuse_r1(struct qcc74x_device_s *dev)
{
    uint32_t reg_val;
 ...
 *
*******************************************************************************/
static void ATTR_TCM_SECTION qcc74x_ef_ctrl_clear_data_reg0(struct qcc74x_device_s *dev)
{
    uint32_t *pefuse_start = (uint32_t *)(QCC74x_EF_CTRL_BASE + 0x00);
    uint32_t i = 0;
...
*******************************************************************************/
#ifdef EF_CTRL_EFUSE_R1_SIZE
static void ATTR_TCM_SECTION qcc74x_ef_ctrl_clear_data_reg1(struct qcc74x_device_s *dev)
{
    uint32_t *pefuse_start = (uint32_t *)(QCC74x_EF_CTRL_BASE + EF_CTRL_EFUSE_R0_SIZE);
    uint32_t i = 0;
...
*******************************************************************************/
#ifdef EF_CTRL_EFUSE_R1_SIZE
static void ATTR_TCM_SECTION qcc74x_ef_ctrl_load_efuse_r1(struct qcc74x_device_s *dev)
{
    uint32_t reg_val;
    ...
    /* Wait for efuse control idle */
    do {
        reg_val = getreg32(QCC74x_EF_CTRL_BASE + EF_CTRL_EF_IF_CTRL_1_OFFSET);
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qcc74x_ef_ctrl_autoload_done(struct qcc74x_device_s *dev)
{
#ifdef romapi_qcc74x_ef_ctrl_autoload_done
    return romapi_qcc74x_ef_ctrl_autoload_done(dev);
#else
    uint32_t reg_val;
 ...
 *
*******************************************************************************/
void ATTR_TCM_SECTION qcc74x_ef_ctrl_write_direct(struct qcc74x_device_s *dev, uint32_t offset, uint32_t *pword, uint32_t count, uint8_t program)
{
#ifdef romapi_qcc74x_ef_ctrl_write_direct
    romapi_qcc74x_ef_ctrl_write_direct(dev, offset, pword, count, program);
#else
 ...
 *
*******************************************************************************/
void ATTR_TCM_SECTION qcc74x_ef_ctrl_read_direct(struct qcc74x_device_s *dev, uint32_t offset, uint32_t *pword, uint32_t count, uint8_t reload)
{
#ifdef romapi_qcc74x_ef_ctrl_read_direct
    romapi_qcc74x_ef_ctrl_read_direct(dev, offset, pword, count, reload);
#else
    uint32_t *pefuse_start = NULL;
    uint32_t region0_count = 0, region1_count = 0;
    uint32_t total_size = EF_CTRL_EFUSE_R0_SIZE;
...
void ATTR_TCM_SECTION qcc74x_ef_ctrl_read_common_trim(struct qcc74x_device_s *dev, char *name, qcc74x_ef_ctrl_com_trim_t *trim, uint8_t reload)
{
    ...
    for (i = 0; i < trim_list_len; i++) {
        if (arch_memcmp(name, trim_list[i].name, qcc74x_ef_ctrl_strlen(name)) == 0) {
            ...
            reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].en_addr / 32) * 4);
            if (reg_val & (1 << (trim_list[i].en_addr % 32))) {
                trim->en = 1;
            }
            reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].parity_addr / 32) * 4);
            if (reg_val & (1 << (trim_list[i].parity_addr % 32))) {
                trim->parity = 1;
            }

            if (((trim_list[i].value_addr % 32) + trim_list[i].value_len) > 32) {
                uint64_t tmpval64 = (uint64_t)getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
                tmpval64 |= (((uint64_t)getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4 + 4)) << 32);
                tmpval64 = tmpval64 >> (trim_list[i].value_addr % 32);
                trim->value = (uint32_t)(tmpval64 & (((uint64_t)1 << trim_list[i].value_len) - 1));
            } else {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
...
void ATTR_TCM_SECTION qcc74x_ef_ctrl_write_common_trim(struct qcc74x_device_s *dev, char *name, uint32_t value, uint8_t program)
{
    ...
    for (i = 0; i < trim_list_len; i++) {
        if (arch_memcmp(name, trim_list[i].name, qcc74x_ef_ctrl_strlen(name)) == 0) {
            ...
            putreg32(reg_val, QCC74x_EF_CTRL_BASE + (trim_list[i].en_addr / 32) * 4);

            parity = qcc74x_ef_ctrl_get_trim_parity(value, trim_list[i].value_len);
            if (parity) {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].parity_addr / 32) * 4);
                reg_val |= (1 << (trim_list[i].parity_addr % 32));
                putreg32(reg_val, QCC74x_EF_CTRL_BASE + (trim_list[i].parity_addr / 32) * 4);
            }

            if (((trim_list[i].value_addr % 32) + trim_list[i].value_len) > 32) {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
                reg_val |= (value << (trim_list[i].value_addr % 32));
                putreg32(reg_val, QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);

                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4) + 4;
                reg_val |= (value >> (32 - (trim_list[i].value_addr % 32)));
                putreg32(reg_val, QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4 + 4);
            } else {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
                reg_val |= (value << (trim_list[i].value_addr % 32));
                putreg32(reg_val, QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
 ...
 *
*******************************************************************************/
uint8_t ATTR_TCM_SECTION qcc74x_ef_ctrl_is_all_bits_zero(uint32_t val, uint8_t start, uint8_t len)
{
#ifdef romapi_qcc74x_ef_ctrl_is_all_bits_zero
    return romapi_qcc74x_ef_ctrl_is_all_bits_zero(val, start, len);
#else
    uint32_t mask = 0;
 ...
 *
*******************************************************************************/
uint32_t ATTR_TCM_SECTION qcc74x_ef_ctrl_get_byte_zero_cnt(uint8_t val)
{
#ifdef romapi_qcc74x_ef_ctrl_get_byte_zero_cnt
    return romapi_qcc74x_ef_ctrl_get_byte_zero_cnt(val);
#else
    uint32_t cnt = 0;
    uint32_t i = 0;
 ...
 *
*******************************************************************************/
uint8_t ATTR_TCM_SECTION qcc74x_ef_ctrl_get_trim_parity(uint32_t val, uint8_t len)
{
#ifdef romapi_qcc74x_ef_ctrl_get_trim_parity
    return romapi_qcc74x_ef_ctrl_get_trim_parity(val, len);
#else
    uint8_t cnt = 0;
    uint8_t i = 0;
