...
void qcc74x_ir_tx_init(struct qcc74x_device_s *dev, const struct qcc74x_ir_tx_config_s *config)
{
    ...
    romapi_qcc74x_ir_tx_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint32_t ir_clock;
    struct qcc74x_ir_tx_config_s *tx_config = (struct qcc74x_ir_tx_config_s *)config;
    ...
    *(uint32_t *)0x20000144 |= 1 << 31;
    ...
    if (qcc74x_clk_get_peripheral_clock(QCC74x_DEVICE_TYPE_IR, 0)) {
        ir_clock = qcc74x_clk_get_peripheral_clock(QCC74x_DEVICE_TYPE_IR, 0);
    } else {
        ir_clock = 2000000;
    }
...
{
#ifdef romapi_qcc74x_ir_send
    romapi_qcc74x_ir_send(dev, data, length);
#else
    uint32_t reg_base;
    uint32_t regval;
#if !defined(QCC74x_undef) && !defined(QCC74x_undef)
    ...
    if ((getreg32(reg_base + IRTX_CONFIG_OFFSET) & IR_CR_IRTX_FRM_EN) == 0) {
        while ((qcc74x_ir_get_txint_status(dev) & IR_TX_INTSTS_END) == 0) {
            /* Waiting for sending */
        }
    } else {
        while (qcc74x_ir_get_txfifo_cnt(dev) < 4) {
            /* Waiting for sending */
        }
    }
#else
...
void qcc74x_ir_tx_enable(struct qcc74x_device_s *dev, bool enable)
{
#ifdef romapi_qcc74x_ir_tx_enable
    romapi_qcc74x_ir_tx_enable(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_ir_txint_mask(struct qcc74x_device_s *dev, uint8_t int_type, bool mask)
{
#ifdef romapi_qcc74x_ir_txint_mask
    romapi_qcc74x_ir_txint_mask(dev, int_type, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_ir_txint_clear
    romapi_qcc74x_ir_txint_clear(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_ir_get_txint_status
    return romapi_qcc74x_ir_get_txint_status(dev);
#else
    uint32_t reg_base;
...
#if !defined(QCC74x_undef) && !defined(QCC74x_undef)
void qcc74x_ir_link_txdma(struct qcc74x_device_s *dev, bool enable)
{
#ifdef romapi_qcc74x_ir_link_txdma
    romapi_qcc74x_ir_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_ir_get_txfifo_cnt
    return romapi_qcc74x_ir_get_txfifo_cnt(dev);
#else
    uint32_t reg_base;
...
{
#ifdef romapi_qcc74x_ir_txfifo_clear
    romapi_qcc74x_ir_txfifo_clear(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_ir_rx_init(struct qcc74x_device_s *dev, const struct qcc74x_ir_rx_config_s *config)
{
    ...
    romapi_qcc74x_ir_rx_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint32_t ir_clock;
...
uint16_t qcc74x_ir_receive(struct qcc74x_device_s *dev, uint64_t *data)
{
    ...
    while ((qcc74x_ir_get_rxint_status(dev) & IR_RX_INTSTS_END) == 0) {
...
void qcc74x_ir_rx_enable(struct qcc74x_device_s *dev, bool enable)
{
#ifdef romapi_qcc74x_ir_rx_enable
    romapi_qcc74x_ir_rx_enable(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_ir_rxint_mask(struct qcc74x_device_s *dev, uint8_t int_type, bool mask)
{
#ifdef romapi_qcc74x_ir_rxint_mask
    romapi_qcc74x_ir_rxint_mask(dev, int_type, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_ir_rxint_clear
    romapi_qcc74x_ir_rxint_clear(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_ir_get_rxint_status
    return romapi_qcc74x_ir_get_rxint_status(dev);
#else
    uint32_t reg_base;
...
{
#ifdef romapi_qcc74x_ir_get_rxfifo_cnt
    return romapi_qcc74x_ir_get_rxfifo_cnt(dev);
#else
    uint32_t reg_base;
...
{
#ifdef romapi_qcc74x_ir_rxfifo_clear
    romapi_qcc74x_ir_rxfifo_clear(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
