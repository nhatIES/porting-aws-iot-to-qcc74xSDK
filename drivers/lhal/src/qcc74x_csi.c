#include "qcc74x_csi.h"
#include "hardware/csi_reg.h"
#include "hardware/dtsrc_reg.h"

#define DTSRC_BASE 0x30012800

static void qcc74x_csi_phy_config(struct qcc74x_device_s *dev, uint32_t tx_clk_escape, uint32_t data_rate)
{
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_csi_init(struct qcc74x_device_s *dev, const struct qcc74x_csi_config_s *config)
{
    uint32_t reg_base;
    uint32_t regval;
    ...
    qcc74x_csi_phy_config(dev, config->tx_clk_escape / 1000000, config->data_rate / 1000000);
...
void qcc74x_csi_start(struct qcc74x_device_s *dev)
{
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_csi_stop(struct qcc74x_device_s *dev)
{
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_csi_set_line_threshold(struct qcc74x_device_s *dev, uint16_t resolution_x, uint32_t pixel_clock, uint32_t dsp_clock)
{
    uint32_t threshold;
...
void qcc74x_csi_int_mask(struct qcc74x_device_s *dev, uint32_t int_type, bool mask)
{
    uint32_t reg_base;
    uint32_t regval;
...
void qcc74x_csi_int_clear(struct qcc74x_device_s *dev, uint32_t int_type)
{
    putreg32(int_type, dev->reg_base + CSI_INT_CLEAR_OFFSET);
}

uint32_t qcc74x_csi_get_intstatus(struct qcc74x_device_s *dev)
{
    return (getreg32(dev->reg_base + CSI_INT_STATUS_OFFSET));
}

int qcc74x_csi_feature_control(struct qcc74x_device_s *dev, int cmd, size_t arg)
{
    int ret = 0;
