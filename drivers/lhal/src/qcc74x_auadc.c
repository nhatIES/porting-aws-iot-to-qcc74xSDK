...
#ifdef romapi_qcc74x_auadc_init
    return romapi_qcc74x_auadc_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
...
#ifdef romapi_qcc74x_auadc_adc_init
    return romapi_qcc74x_auadc_adc_init(dev, adc_analog_cfg);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_auadc_link_rxdma
    return romapi_qcc74x_auadc_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_auadc_get_intstatus
    return romapi_qcc74x_auadc_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
    int32_t int_sts;
...
{
#ifdef romapi_qcc74x_auadc_feature_control
    return romapi_qcc74x_auadc_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
