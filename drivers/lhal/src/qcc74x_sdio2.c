...
struct qcc74x_sdio2_queue_ctrl_s {
    ...
    qcc74x_sdio2_trans_desc_t sdio_dnld_port_table[SDIO2_MAX_PORT_NUM];

    uint32_t sdio_upld_port_head;
    uint32_t sdio_upld_port_tail;
    qcc74x_sdio2_trans_desc_t sdio_upld_port_table[SDIO2_MAX_PORT_NUM];
};

/* isr event callback */
static void *sdio2_irq_arg = NULL;
static qcc74x_sdio2_irq_cb_t sdio2_irq_event_cb = NULL;
...
#if defined(SDIO2_SOFT_RST_INT_SUP)
    qcc74x_irq_attach(SDIO2_SOFT_RST_IRQ_NUM, qcc74x_sdio2_reset_isr, dev);
    qcc74x_irq_enable(SDIO2_SOFT_RST_IRQ_NUM);
#endif
...
#if defined(SDIO2_SOFT_RST_INT_SUP)
    qcc74x_irq_disable(SDIO2_SOFT_RST_IRQ_NUM);
#endif
...
int qcc74x_sdio2_dnld_port_push(struct qcc74x_device_s *dev, qcc74x_sdio2_trans_desc_t *trans_desc)
{
    ...
    if (trans_desc->buff_len < qcc74x_sdio2_feature_control(dev, SDIO2_CMD_GET_DNLD_MAX_SIZE, 0)) {
    ...
    qcc74x_sdio2_trig_host_int(dev, SDIO2_HOST_INT_EVENT_DNLD_RDY);
...
int qcc74x_sdio2_upld_port_push(struct qcc74x_device_s *dev, qcc74x_sdio2_trans_desc_t *trans_desc)
{
    ...
    if (trans_desc->data_len > SDIO2_BYTE_MOD_SIZE_MAX) {
        ...
        uint16_t block_size = (uint16_t)qcc74x_sdio2_feature_control(dev, SDIO2_CMD_GET_FUNC_BLK_SIZE, 0);
    ...
    blk_size = (uint16_t)qcc74x_sdio2_feature_control(dev, SDIO2_CMD_GET_FUNC_BLK_SIZE, 0);
    ...
    /* trig dnld card2host int */
    qcc74x_sdio2_trig_host_int(dev, SDIO2_HOST_INT_EVENT_UPLD_RDY);
    ...
    if ((regval & SDIO2_CCR_CIC_UpLdOvr)) {
        ...
        for (uint8_t i = 0; i < SDIO2_MAX_PORT_NUM; i++) {
            if (sdio2_ctrl.sdio_upld_port_head == sdio2_ctrl.sdio_upld_port_tail) {
                ...
                /*  */
                qcc74x_sdio2_trans_desc_t upld_trans_desc = sdio2_ctrl.sdio_upld_port_table[sdio2_ctrl.sdio_upld_port_head & SDIO2_MAX_PORT_MASK];
    ...
    if ((regval & SDIO2_CCR_CIC_DnLdOvr) &&
        ...
        for (uint8_t i = 0; i < SDIO2_MAX_PORT_NUM; i++) {
            if (sdio2_ctrl.sdio_dnld_port_head == sdio2_ctrl.sdio_dnld_port_tail) {
                ...
                /*  */
                qcc74x_sdio2_trans_desc_t trans_desc = sdio2_ctrl.sdio_dnld_port_table[sdio2_ctrl.sdio_dnld_port_head & SDIO2_MAX_PORT_MASK];
...
static void qcc74x_sdio2_reset_isr(int irq, void *arg)
{
    qcc74x_irq_clear_pending(SDIO2_SOFT_RST_IRQ_NUM);
