...
static void lwip_emac_irq_cb(void *arg, uint32_t irq_event, struct qcc74x_emac_trans_desc_s *trans_desc);
...
void lwip_emac_if_cfg(eth_phy_init_cfg_t *phy_cfg_new, struct qcc74x_emac_config_s *emac_cfg_new)
{
    if (phy_cfg_new) {
        phy_cfg = *phy_cfg_new;
    }
...
static int emac_low_level_init(struct netif *netif)
{
    ...
    qcc74x_emac_irq_attach(emac0, lwip_emac_irq_cb, NULL);
    ...
    /* tx pool queue init */
    tx_pool_queue = xQueueCreate(LWIP_EMAC_TX_BUFF_CNT, sizeof(struct qcc74x_emac_trans_desc_s));

    /* rx process queue init */
    rx_process_queue = xQueueCreate(LWIP_EMAC_RX_BUFF_CNT, sizeof(struct qcc74x_emac_trans_desc_s));
...
{
    struct pbuf *q;
    struct qcc74x_emac_trans_desc_s trans_desc;
    ...
    tx_push_cnt += 1;
    qcc74x_emac_queue_tx_push(emac0, &trans_desc);
