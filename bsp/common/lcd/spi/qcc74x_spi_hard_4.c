...
/* dma device */
static struct qcc74x_device_s *spi_dma_hd;
/* spi */
static struct qcc74x_device_s *spi_hd;
/* goio */
static struct qcc74x_device_s *gpio;

/* The memory space of DMA */
static struct qcc74x_dma_channel_lli_pool_s dma_tx_llipool[LCD_SPI_HARD_4_DMA_LLI_NUM];
...
static void spi_dma_callback(void *arg)
{
    ...
    while (qcc74x_spi_isbusy(spi_hd)) {
        __ASM volatile("nop");
    };

    /* Switch the SPI to non-DMA mode */
    qcc74x_spi_link_txdma(spi_hd, false);

    /* clear rx fifo */
    qcc74x_spi_feature_control(spi_hd, SPI_CMD_CLEAR_RX_FIFO, 0);

    LCD_SPI_HARD_4_CS_HIGH;

    /* 8-bit data */
    qcc74x_spi_feature_control(spi_hd, SPI_CMD_SET_DATA_WIDTH, SPI_DATA_WIDTH_8BIT);
    ...
    if (spi_async_cycle_fill_flag == true) {
        ...
        qcc74x_dma_feature_control(spi_dma_hd, DMA_CMD_SET_SRCADDR_INCREMENT, true);
    }
...
int lcd_spi_hard_4_init(lcd_spi_hard_4_init_t *dbi_parra)
{
    ...
    /* dma cfg */
    struct qcc74x_dma_channel_config_s dma_spi_tx_cfg = {
    ...
    spi_hd = qcc74x_device_get_by_name(LCD_SPI_HARD_4_NAME);

    /* CS and DC pin init */
    gpio = qcc74x_device_get_by_name("gpio");
    qcc74x_gpio_init(gpio, LCD_SPI_HARD_4_PIN_CS, GPIO_OUTPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_1);
    qcc74x_gpio_init(gpio, LCD_SPI_HARD_4_PIN_DC, GPIO_OUTPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_1);
    LCD_SPI_HARD_4_CS_HIGH;
    LCD_SPI_HARD_4_DC_HIGH;

    if (spi_hd->idx == 0) {
        qcc74x_gpio_init(gpio, LCD_SPI_HARD_4_PIN_CLK, GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_2);
        qcc74x_gpio_init(gpio, LCD_SPI_HARD_4_PIN_DAT, GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_2);
    }
#if defined(GPIO_FUNC_SPI1)
    else if (spi_hd->idx == 1) {
        qcc74x_gpio_init(gpio, LCD_SPI_HARD_4_PIN_CLK, GPIO_FUNC_SPI1 | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_2);
        qcc74x_gpio_init(gpio, LCD_SPI_HARD_4_PIN_DAT, GPIO_FUNC_SPI1 | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_2);
    }

#endif

    /* spi init */
    qcc74x_spi_init(spi_hd, &spi_cfg);

    /* spi enabled continuous mode */
    // qcc74x_spi_feature_control(spi_hd, SPI_CMD_SET_CS_INTERVAL, true);
    ...
    /* dma init */
    spi_dma_hd = qcc74x_device_get_by_name(LCD_SPI_HARD_4_DMA_NAME);
    qcc74x_dma_channel_init(spi_dma_hd, &dma_spi_tx_cfg);

    /* dma int cfg */
    qcc74x_dma_channel_irq_attach(spi_dma_hd, spi_dma_callback, NULL);
...
int lcd_spi_hard_4_transmit_cmd_para(uint8_t cmd, uint32_t *para, size_t para_num)
{
    ...
    /* send cmd */
    qcc74x_spi_poll_send(spi_hd, cmd);

    LCD_SPI_HARD_4_DC_HIGH;

    /* send para */
    if (para_num && para != NULL) {
        qcc74x_spi_poll_exchange(spi_hd, para, NULL, para_num);
    }
...
int lcd_spi_hard_4_transmit_cmd_pixel_sync(uint8_t cmd, uint32_t *pixel, size_t pixel_num)
{
    ...
    if (pixel_num && pixel != NULL) {
        if (pixel_format == LCD_SPI_LCD_PIXEL_FORMAT_RGB565) {
            ...
            qcc74x_spi_feature_control(spi_hd, SPI_CMD_SET_DATA_WIDTH, SPI_DATA_WIDTH_16BIT);
        }

        /* send pixel */
        if (pixel_format == LCD_SPI_LCD_PIXEL_FORMAT_RGB565) {
            qcc74x_spi_poll_exchange(spi_hd, pixel, NULL, pixel_num * 2);
        }
...
int lcd_spi_hard_4_transmit_cmd_pixel_async(uint8_t cmd, uint32_t *pixel, size_t pixel_num)
{
    ...
    if (pixel_num && pixel != NULL) {
        ...
        /* enable spi dma mode */
        qcc74x_spi_link_txdma(spi_hd, true);
        ...
        if (pixel_format == LCD_SPI_LCD_PIXEL_FORMAT_RGB565) {
            ...
            /* clean cache */
            qcc74x_l1c_dcache_clean_range((void *)pixel, pixel_num * 2);
        }

        qcc74x_dma_channel_lli_reload(spi_dma_hd, dma_tx_llipool, LCD_SPI_HARD_4_DMA_LLI_NUM, dma_tx_transfers, 1);
        qcc74x_dma_channel_start(spi_dma_hd);
    } else {
        LCD_SPI_HARD_4_CS_HIGH;
    }
    return 0;
...
int lcd_spi_hard_4_transmit_cmd_pixel_fill_sync(uint8_t cmd, uint32_t pixel_val, size_t pixel_num)
{
    ...
    if (pixel_num) {
        ...
        for (size_t i = 0; i < pixel_num; i++) {
            qcc74x_spi_poll_send(spi_hd, pixel_val);
        }
...
int lcd_spi_hard_4_transmit_cmd_pixel_fill_async(uint8_t cmd, uint32_t pixel_val, size_t pixel_num)
{
    ...
    if (pixel_num) {
        ...
        qcc74x_dma_feature_control(spi_hd, DMA_CMD_SET_SRCADDR_INCREMENT, false);
