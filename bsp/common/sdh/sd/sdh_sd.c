...
{
    int ret;
    struct qcc74x_sdh_cmd_cfg_s cmd_cfg = { 0 };
...
int sdh_sd_send_csd(struct sd_card_s *sd_card)
{
    ...
    csd->read_qcc74x_len = (raw_csd[2] >> 16) & 0x0f;     /*!< Maximum read data block length [83:80] */
    csd->read_qcc74x_partial = (raw_csd[2] >> 15) & 0x01; /*!< Partial blocks for read allowed [79:79] */
    ...
    if (csd->csd_structure == 0) {
        ...
        sd_card->block_size = 1 << csd->read_qcc74x_len;
    ...
    csd->write_qcc74x_len = (raw_csd[0] >> 22) & 0x0F;       /*!< Maximum write data block length [25:22] */
    csd->write_qcc74x_partial = (raw_csd[0] >> 21) & 0x01;   /*!< Partial blocks for write allowed [21:21] */
    ...
    LOG_D("CSD READ_QCC74x_LEN: %d Byte\r\n", 1 << csd->read_qcc74x_len);
    LOG_D("CSD READ_QCC74x_PARTIAL: %s\r\n", csd->read_qcc74x_partial ? "ture" : "false");
...
int sdh_sd_send_scr(struct sd_card_s *sd_card)
{
    int ret;
    struct qcc74x_sdh_cmd_cfg_s cmd_cfg = { 0 };
    struct qcc74x_sdh_data_cfg_s data_cfg = { 0 };
    ...
    cmd_cfg.resp_type = SDH_RESP_TPYE_R1;
    /* data cfg */
    struct qcc74x_sdh_data_tranfer_s data_tranfer = {
...
int sdh_sd_read_blocks(struct sd_card_s *sd_card, void *rd_buff, uint32_t blk_addr, uint32_t blk_cnt)
{
    ...
    qcc74x_l1c_dcache_clean_range(rd_buff, 1);
    qcc74x_l1c_dcache_clean_range((rd_buff + SD_DEFAULT_BLOCK_SIZE * blk_cnt), 1);
    qcc74x_l1c_dcache_invalidate_range(rd_buff, SD_DEFAULT_BLOCK_SIZE * blk_cnt);
...
int sdh_sd_write_blocks(struct sd_card_s *sd_card, void *wr_buff, uint32_t blk_addr, uint32_t blk_cnt)
{
    ...
    qcc74x_l1c_dcache_clean_range(wr_buff, SD_DEFAULT_BLOCK_SIZE * blk_cnt);
