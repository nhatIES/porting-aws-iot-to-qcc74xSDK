...
static __ALIGNED(32) ATTR_NOCACHE_RAM_SECTION struct qcc74x_sdh_adma2_hw_desc_s adam2_hw_desc[SDH_HOST_ADMA2_HW_DESC_NUM];
...
static void sdh_irq(int irq, void *arg)
{
    ...
    qcc74x_sdh_sta_int_en(host->sdh_dev, 0xffffffff, false);
    /* give semb */
...
int sdh_host_init(struct sdh_host_s *host)
{
    ...
    host->sdh_dev = qcc74x_device_get_by_name("sdh");
    ...
    /* enable cpu irq */
    qcc74x_irq_attach(host->sdh_dev->irq_num, sdh_irq, host);
    qcc74x_irq_enable(host->sdh_dev->irq_num);
#endif
...
#elif defined(QCC74x_undef)
    /* clock en */
    qcc74x_glb_set_sdh_clk(true, GLB_SDH_CLK_WIFIPLL_96M, 0);
    PERIPHERAL_CLOCK_SDH_ENABLE();
    /* sdh reset */
    qcc74x_glb_ahb_mcu_software_reset(GLB_AHB_MCU_SW_EXT_SDH);
...
#error [SDH] unkown chip_ID
    ...
    /* init sdh */
    struct qcc74x_sdh_config_s cfg = {
        .dma_burst = SDH_DMA_BURST_64,
        .dma_fifo_th = SDH_DMA_FIFO_THRESHOLD_128,
        .power_vol = 0x07, /* 3.3v */
    };
    qcc74x_sdh_init(host->sdh_dev, &cfg);

    sdh_set_bus_clock(host, 400 * 1000, NULL);

    /* sd bus enable */
    qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_SD_BUS_POWER, true);
...
int sdh_host_transfer(struct sdh_host_s *host, struct sdh_host_transfer_s *transfer)
{
    ...
    /* clr comp sta */
    qcc74x_sdh_sta_clr(host->sdh_dev, SDH_NORMAL_STA_CMD_COMP | SDH_NORMAL_STA_TRAN_COMP);
    /* clr all error sta */
    qcc74x_sdh_sta_clr(host->sdh_dev, SDH_ERROR_ALL_MASK);

    /*  */
    ret = qcc74x_sdh_tranfer_start(host->sdh_dev, transfer->cmd_cfg, transfer->data_cfg);
...
int sdh_host_wait_done(struct sdh_host_s *host, struct sdh_host_transfer_s *transfer)
{
    ...
    /* Polling mode */
    do {
        sta = qcc74x_sdh_sta_get(host->sdh_dev);
    } while ((sta & (SDH_NORMAL_STA_CMD_COMP | SDH_NORMAL_STA_ERROR)) == 0);
#else
    /* Interrupt mode */
    qcc74x_sdh_sta_int_en(host->sdh_dev, SDH_NORMAL_STA_CMD_COMP | SDH_ERROR_ALL_MASK, true);
    ...
    if (sta & SDH_NORMAL_STA_CMD_COMP) {
        qcc74x_sdh_sta_clr(host->sdh_dev, SDH_NORMAL_STA_CMD_COMP);
        qcc74x_sdh_get_resp(host->sdh_dev, transfer->cmd_cfg);
    } else {
        ret = -1;
        goto wait_exit;
    }
...
#else
    /* Interrupt mode */
    qcc74x_sdh_sta_int_en(host->sdh_dev, SDH_NORMAL_STA_TRAN_COMP | SDH_ERROR_ALL_MASK, true);
    sdh_osal_semb_take(host->osal_semb, 0);
    ...
    if (sta & SDH_NORMAL_STA_TRAN_COMP) {
        qcc74x_sdh_sta_clr(host->sdh_dev, SDH_NORMAL_STA_TRAN_COMP);
        ret = 0;
        goto wait_exit;
    } else {
        ret = -2;
...
int sdh_set_bus_width(struct sdh_host_s *host, uint8_t bus_width)
{
    ...
    ret = qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_BUS_WIDTH, bus_width);
    return ret;
}
...
int sdh_set_bus_clock(struct sdh_host_s *host, uint32_t freq, uint32_t *actual_freq)
{
    ...
    ret = qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_BUS_CLK_EN, false);
    ...
    qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_BUS_CLK_DIV, 0);
    ...
    host->sdh_div = div;
    qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_BUS_CLK_DIV, div - 1);
#endif
    ...
    if (actual_freq_hz > 25 * 1000 * 1000) {
        qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_HS_MODE_EN, true);
    } else {
        qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_HS_MODE_EN, false);
    }
#endif

    ret = qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_BUS_CLK_EN, true);
    return ret;
}
...
{
#ifdef SDH_STD_V3
    qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_ACTIVE_CLK_OUT, 80);
...
int sdh_set_data_timeout_ms(struct sdh_host_s *host, uint16_t timeout_ms, uint32_t *actual_ms)
{
    ...
    ret = qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_SET_DATA_TIMEOUT_CNT_VAL, cnt_vaule);
...
{
#ifdef SDH_STD_V3
    qcc74x_sdh_feature_control(host->sdh_dev, SDH_CMD_FORCE_CLK_OUTPUT, en);
#endif
    return 0;
}
