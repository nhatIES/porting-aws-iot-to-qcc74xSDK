...
#else
    /* no os */
    qcc74x_mtimer_delay_ms(delay_ms);
#endif
}
...
#else
    /* no os */
    uint32_t time = qcc74x_mtimer_get_time_ms();
    while (*((volatile bool *)semb) == false) {
        if (timeout_ms && (qcc74x_mtimer_get_time_ms() - time > timeout_ms)) {
            break;
        }
    };
    uintptr_t irq_flag = qcc74x_irq_save();
