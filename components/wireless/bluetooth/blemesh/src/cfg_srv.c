...
void bt_mesh_app_key_del(struct bt_mesh_app_key *key, bool store)
{
    ...
    #if defined(QCC74x_BLE_MESH_PATCH_DEL_APPKEY)
...
#if !defined(QCC74x_BLE_MESH_PATCH_MOD_PUB_VA_SET)
		      ...
		      bool vnd, bool primary, void *user_data)
{
	...
	if (mod->cb && mod->cb->reset) {
		...
		/** Added by qcc74x lab, when reset callback dont't set, 
		 *  call mod bind.
		 * */
		 if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
...
void bt_mesh_heartbeat(u16_t src, u16_t dst, u8_t hops, u16_t feat)
{
	...
	if (cfg->hb_sub.func) {
		cfg->hb_sub.func(src, dst, hops, feat);/* Modified by qcc74x */
	}
}
