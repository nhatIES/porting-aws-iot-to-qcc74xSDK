		   ...
		   void *cb_arg)
{
	...
	if (CONFIG_BT_MESH_SEQ_STORE_RATE > 0) {
		...
		bt_mesh.seq += (CONFIG_BT_MESH_SEQ_STORE_RATE -
				(bt_mesh.seq % CONFIG_BT_MESH_SEQ_STORE_RATE))
				+ BT_MESH_SEQ_PWRON_COMPENSATE; /* Add by qcc74x */
		bt_mesh.seq--;
	}
...
static int cdb_net_set(const char *name, size_t len_rd,
		       settings_read_cb read_cb, void *cb_arg)
{
	struct cdb_net_val net = {};/*Modified by qcc74x */
	int err;
...
static int cdb_set(const char *name, size_t len_rd,
		   settings_read_cb read_cb, void *cb_arg)
{
	int len = 0; /* Add by qcc74x */
	const char *next = NULL; /* Add by qcc74x */
...
#ifdef CONFIG_BT_SETTINGS /* Added by qcc74x */
