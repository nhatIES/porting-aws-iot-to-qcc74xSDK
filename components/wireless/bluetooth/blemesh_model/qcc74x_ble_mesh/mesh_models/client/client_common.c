...
static s32_t bt_mesh_get_adv_duration(void)
{
    ...
    adv_int = BT_MESH_TRANSMIT_INT(xmit);
    duration = (BT_MESH_TRANSMIT_COUNT(xmit) + 1) * (adv_int + 10);
                                         ...
                                         u32_t opcode, s32_t timeout)
{
    ...
    mic_size = (need_seg && net_buf_simple_tailroom(msg) >= QCC74x_BLE_MESH_MIC_LONG) ?
                QCC74x_BLE_MESH_MIC_LONG : QCC74x_BLE_MESH_MIC_SHORT;
                            ...
                            void *cb_data)
{
    ...
    node->timeout = bt_mesh_client_calc_timeout(ctx, msg, opcode, timeout ? timeout : 1000);
