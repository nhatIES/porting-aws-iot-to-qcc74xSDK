...
int bt_mesh_get_light_lc_trans_time(struct bt_mesh_model *model, u8_t *trans_time)
{
    ...
    switch(srv->lc->state_machine.state){
        case LC_OFF:
            ...
            break;
        case LC_STANDBY:
            value = srv->lc->prop_state.lightness_standby & 0xFFFFFF;
            break;
        case LC_FADE_ON:
            value = srv->lc->prop_state.time_fade_on & 0xFFFFFF;
            break;
        case LC_RUN:
            value = srv->lc->prop_state.time_run_on & 0xFFFFFF;
            break;
        case LC_FADE:
            value = srv->lc->prop_state.time_fade & 0xFFFFFF;
            break;
        case LC_PROLONG:
            value = srv->lc->prop_state.time_prolong & 0xFFFFFF;
            break;
        case LC_FADE_STANDBY_AUTO:
            value = srv->lc->prop_state.time_fade_standby_auto & 0xFFFFFF;
            break;
        case LC_FADE_STANDBY_MANUAL:
             value = srv->lc->prop_state.time_fade_standby_manual & 0xFFFFFF;
            break;
        default:
            value = 0;
            break;
