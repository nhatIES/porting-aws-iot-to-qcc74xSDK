                                        ...
                                        bool publish, u16_t opcode)
{
    ...
    case BLE_MESH_MODEL_OP_LIGHT_LIGHTNESS_DEFAULT_STATUS:
        if (model->id == BT_MESH_MODEL_ID_LIGHT_LIGHTNESS_SRV) {
            struct bt_mesh_light_lightness_srv *srv = model->user_data;
            net_buf_simple_add_le16(msg, srv->state->lightness_default);
        } else if (model->id == BT_MESH_MODEL_ID_LIGHT_LIGHTNESS_SETUP_SRV) {
                                  ...
                                  bool publish, u16_t opcode)
{
    ...
    case BLE_MESH_MODEL_OP_LIGHT_XYL_DEFAULT_STATUS:
        if (model->id == BT_MESH_MODEL_ID_LIGHT_XYL_SRV) {
            struct bt_mesh_light_xyl_srv *srv = model->user_data;
            net_buf_simple_add_le16(msg, srv->state->lightness_default);
            net_buf_simple_add_le16(msg, srv->state->x_default);
            net_buf_simple_add_le16(msg, srv->state->y_default);
        } else if (model->id == BT_MESH_MODEL_ID_LIGHT_XYL_SETUP_SRV) {
                         ...
                         struct net_buf_simple *buf)
{
    ...
    /* Only for MMDL/SR/LLC/BV-04-C */
    if(ctx->recv_op == BLE_MESH_MODEL_OP_LIGHT_LC_LIGHT_ONOFF_GET){
        extern uint8_t mmdl_test_val_trigger;
        BT_WARN("send_light_lc_status %x", srv->lc->state.light_onoff);
        if(mmdl_test_val_trigger && srv->lc->state.light_onoff == 1){
            mmdl_test_val_trigger = 0;
            srv->lc->state.light_onoff = 0;
        }
    }
#endif /* CONFIG_AUTO_PTS */
    return;
...
{
    bt_mesh_lc_state* state = &srv->lc->state_machine.state;
    BT_WARN("state_machine[%x] and do action ",*state);
    switch(*state){
        case LC_STANDBY:
            //k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.lightness_standby & 0xFFFFFF);
            srv->lc->state.light_onoff = 0;
            break;
        case LC_FADE_ON:
            k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.time_fade_on & 0xFFFFFF);
            break;
        case LC_RUN:
            k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.time_run_on & 0xFFFFFF);
            break;
        case LC_FADE:
            k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.time_fade & 0xFFFFFF);
            break;
        case LC_PROLONG:
            k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.time_prolong & 0xFFFFFF);
            break;
        case LC_FADE_STANDBY_AUTO:
            *state = LC_STANDBY;
            k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.time_fade_standby_auto & 0xFFFFFF);
            break;
        case LC_FADE_STANDBY_MANUAL:
            k_delayed_work_submit(&state_machine_work, srv->lc->prop_state.time_fade_standby_manual & 0xFFFFFF);
            break;
        default:
            break;
    }
...
void state_machine_timer(struct bt_mesh_light_lc_srv * srv)
{
    bt_mesh_lc_state* state = &srv->lc->state_machine.state;
    switch(*state){
        case LC_STANDBY:
...
static void state_machine_work_handler_t(struct k_work *work)
{
    BT_WARN("state_machine_work_handler_t");
    state_machine_timer((struct bt_mesh_light_lc_srv *)work->_reserved);
}
#endif /* CONFIG_AUTO_PTS */
...
int bt_mesh_light_lightness_srv_init(struct bt_mesh_model *model)
{
    ...
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_LIGHT_LIGHTNESS_SETUP_SRV) == NULL) {
...
int bt_mesh_light_ctl_srv_init(struct bt_mesh_model *model)
{
     ...
     */
    struct bt_mesh_elem *element = bt_mesh_model_elem(model);
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_LIGHT_CTL_SETUP_SRV) == NULL) {
...
int bt_mesh_light_hsl_srv_init(struct bt_mesh_model *model)
{
     ...
     */
    struct bt_mesh_elem *element = bt_mesh_model_elem(model);
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_LIGHT_HSL_SETUP_SRV) == NULL) {
...
int bt_mesh_light_xyl_srv_init(struct bt_mesh_model *model)
{
     ...
     */
    struct bt_mesh_elem *element = bt_mesh_model_elem(model);
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_LIGHT_XYL_SETUP_SRV) == NULL) {
...
int bt_mesh_light_lc_setup_srv_init(struct bt_mesh_model *model)
{
     ...
     */
    struct bt_mesh_elem *element = bt_mesh_model_elem(model);
    if (bt_mesh_model_find(element, BT_MESH_MODEL_ID_LIGHT_LC_SETUP_SRV) == NULL) {
...
int bt_mesh_light_lightness_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light Lightness Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_lightness_setup_srv_deinit(struct bt_mesh_model *model)
{
    return light_server_deinit(model);
}

int bt_mesh_light_ctl_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light CTL Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_ctl_setup_srv_deinit(struct bt_mesh_model *model)
{
    return light_server_deinit(model);
}

int bt_mesh_light_ctl_temp_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light CTL Temperature Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_hsl_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light HSL Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_hsl_setup_srv_deinit(struct bt_mesh_model *model)
{
    return light_server_deinit(model);
}

int bt_mesh_light_hsl_hue_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light HSL Hue Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_hsl_sat_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light HSL Saturation Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_xyl_srv_deinit(struct bt_mesh_model *model)
{
    if (model->pub == NULL) {
        BT_ERR("%s, Light xyL Server has no publication support", __func__);
        return -EINVAL;
...
int bt_mesh_light_xyl_setup_srv_deinit(struct bt_mesh_model *model)
{
    return light_server_deinit(model);
}
...
const struct bt_mesh_model_cb bt_mesh_light_lc_srv_cb = {
	.init = bt_mesh_light_lc_srv_init,
};

const struct bt_mesh_model_cb bt_mesh_light_lc_setup_srv_cb = {
	.init = bt_mesh_light_lc_setup_srv_init,
};
