...
static u8_t oad_write_flash(uint32_t filesize, uint32_t offset,u8_t *data, u16_t len)
{
    ...
    if (!oad_env.new_img_addr){
        ...
        if(oad_env.upgrd_file_size <= size){
            BT_WARN("flash erase\r\n");
            qcc74x_flash_erase(oad_env.new_img_addr, oad_env.upgrd_file_size);
        }else{
            return -1;
        }
    }
    ...
    qcc74x_flash_write(wflash_address, data, len);
...
#define CHECK_IMG_BUF_SIZE 512
    ...
    while (offset < bin_size) {
        ...
        if (qcc74x_flash_read(oad_env.new_img_addr+offset, r_buf, read_size)) {
    ...
    qcc74x_flash_read(hash_addr,sha256_img,32);
    for (i = 0; i < 32; i++) {
