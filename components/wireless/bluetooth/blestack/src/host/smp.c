...
#if defined(QCC74x_BLE_SMP_LOCAL_AUTH)
...
#if defined(QCC74x_BLE_SMP_SC_ONLY)
...
static u8_t smp_master_ident(struct bt_smp *smp, struct net_buf *buf)
{
	...
	if (atomic_test_bit(smp->flags, SMP_FLAG_BOND)) {
		...
		#if !defined(QCC74x_BLE_PATCH_CLEAR_REMOTE_KEY_BIT)
		smp->remote_dist &= ~BT_SMP_DIST_ENC_KEY;
		#endif
	}
				  ...
				  u8_t hci_status)
{
	...
	if (hci_status) {
		#ifdef QCC74x_BLE_AUTO_CLEAN_KEY_WHEN_KEY_MISSING
		...
		#endif /* QCC74x_BLE_AUTO_CLEAN_KEY_WHEN_KEY_MISSING */
		return;
	}
