...
{
#ifndef QCC74x_MFD_DECRYPT_BUF_LEN
#define QCC74x_MFD_DECRYPT_BUF_LEN 512
#endif
    uint32_t mfd_decrypt_buf[QCC74x_MFD_DECRYPT_BUF_LEN / sizeof(uint32_t)];
 ...
 *  */
    ...
    if (cipher_size) {
        crc_value = QCC74x_Soft_CRC32((uint8_t *)(xipaddr_base + 4), cipher_size);
        if (crc_value != *(uint32_t *)(xipaddr_base + 4 + cipher_size)) {
            return false;
        }
        if (cipher_size > QCC74x_MFD_DECRYPT_BUF_LEN) {
            return false;
        }
    }

    crc_value = QCC74x_Soft_CRC32((uint8_t *)((xipaddr_base + 4 + cipher_size + 4) + 4), plaintext_size);
    ...
    if (cipher_size) {
        if (false == qcc74x_mfd_decrypt((uint8_t *)(xipaddr_base + 4), cipher_size, (uint8_t *)mfd_decrypt_buf, iv)) {
            return false;
        }
