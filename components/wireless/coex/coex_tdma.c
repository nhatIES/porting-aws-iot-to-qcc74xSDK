...
#if COEX_DBG_LEVEL > 0
#define COEX_INFO(fmt, ...) \
        printf("[Coex][Info] %s "fmt"\r\n", __FUNCTION__, ##__VA_ARGS__)
#else
#define COEX_INFO(fmt, ...)
#endif

#if COEX_DBG_LEVEL > 1
#define COEX_WARN(fmt, ...) \
        printf("[Coex][Warn] %s "fmt"\r\n", __FUNCTION__, ##__VA_ARGS__)
#else
#define COEX_WARN(fmt, ...)
#endif

#if COEX_DBG_LEVEL > 2
#define COEX_ERROR(fmt, ...) \
        printf("[Coex][Error] %s "fmt"\r\n", __FUNCTION__, ##__VA_ARGS__)
#else
#define COEX_ERROR(fmt, ...)
#endif
...
{
    uint32_t yield = ctxt->duration - qcc74x_timer_get_countervalue(coex_env.tmr);
...
{
    bool status = qcc74x_timer_get_compint_status(coex_env.tmr, TIMER_COMP_ID_0);

    if (status) {
        // Clear interrupt
        qcc74x_timer_compint_clear(coex_env.tmr, TIMER_COMP_ID_0);

        // Check validity
        uint64_t now = qcc74x_mtimer_get_time_us();
        ...
        COEX_INFO("send ok: %d", (pdPASS == ret));
    }
}
...
static void coex_tdma_tmr_init(void)
{
    ...
    coex_env.cfg.clock_div       = 39;                         /* for qcc743/qcc74x_undef/qcc74x_undefp is 39, for qcc74x_undef is 31 */
    ...
    // Get timer
    coex_env.tmr = qcc74x_device_get_by_name("timer0");
}
...
static void coex_tdma_tmr_start(struct rf_ctxt_tag* ctxt, uint32_t yield_time)
{
    ...
    qcc74x_timer_init(coex_env.tmr, &coex_env.cfg);
    qcc74x_irq_attach(coex_env.tmr->irq_num, coex_tdma_tmr_isr, ctxt);
    qcc74x_irq_enable(coex_env.tmr->irq_num);

    qcc74x_timer_start(coex_env.tmr);
...
{
    qcc74x_irq_disable(coex_env.tmr->irq_num);
    qcc74x_timer_stop(coex_env.tmr);
}
...
static void coex_tdma_ctxt_switch(struct rf_ctxt_tag* ctxt, int start_timer, uint32_t yield_time)
{
    ...
    coex_env.valid_isr_msg_time = qcc74x_mtimer_get_time_us();
...
static void coex_tdma_ctxt_dump(int state)
{
    for (int i = 0; i < COEX_ROLE_MAX; i++)
    {
        ...
        COEX_CLI("     ctxt, role: %u, act_lvl: %u, credits: %u, dur: %u, fix_dur: %d, times: %d, exclusive: %u",
                 rf_ctxt[i].role, rf_ctxt[i].activity_level, rf_ctxt[i].credits, rf_ctxt[i].duration,
                 rf_ctxt[i].fixed_duration, rf_ctxt[i].fixed_times, rf_ctxt[i].single_exclusive);
    }
}

static void coex_tdma_info_dump(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    ...
    COEX_CLI("Coex info:");
    COEX_CLI("     state: %u, paused: %u, duty_cycle: %u, cur_ctxt: %p, fixed_ctxt: %p, fixed_times: %d, valid_time: %u",
             coex_env.state, coex_env.paused, coex_env.duty_cycle, coex_env.cur_ctxt,
...
{
    COEX_INFO("dummy ctxt notify cb, event: %d", args->event);
}
...
SHELL_CMD_EXPORT_ALIAS(coex_tdma_info_dump, coex_dump, coex module info dump);
...
int coex_event_tdma(struct coex_evt_arg* arg)
{
    ...
    {
        param->arg.set_act.now = qcc74x_mtimer_get_time_us();
    }
#endif
