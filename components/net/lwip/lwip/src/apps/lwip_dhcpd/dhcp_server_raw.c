                ...
                if (msg_type == DHCP_REQUEST)
                {
                    ...
                    if (node != NULL)
                    {
                        ...
                        *opt_buf++ = DHCP_OPTION_END;
                        ...
                        struct netif netif;
                        SMEMCPY(&netif.ip_addr, &node->ipaddr, sizeof(netif.ip_addr));
...
void dhcpd_start(struct netif *netif, int start, int limit)
{
...

    {
        ...
        snprintf(p, IP4ADDR_STRLEN_MAX - (p - str_tmp), "%d", start_num);
        ip4addr_aton(str_tmp, &ip_start);
        DEBUG_PRINTF("ip_start: [%s]\r\n", str_tmp);
        snprintf(p, IP4ADDR_STRLEN_MAX - (p - str_tmp), "%d", end_num);
