...
static void _uart_hw_init(const netbus_uart_config_t *config)
{
    ...
    pdev = qcc74x_device_get_by_name(config->name);
    qcc74x_uart_init(pdev, &cfg);

    qcc74x_uart_link_txdma(pdev, true);
    qcc74x_uart_link_rxdma(pdev, true);
}
...
static void _uart_dma_init(netbus_uart_ctx_t *ctx)
{
    ...
    ctx->dma_tx_chan = qcc74x_device_get_by_name(ctx->config->tx_dmach);
    ctx->dma_rx_chan = qcc74x_device_get_by_name(ctx->config->rx_dmach);

    qcc74x_dma_channel_init(ctx->dma_tx_chan, &tx_config);
    qcc74x_dma_channel_init(ctx->dma_rx_chan, &rx_config);

    qcc74x_dma_channel_irq_attach(ctx->dma_tx_chan, _uart_dma_tx_irq, (void *)ctx);
    qcc74x_dma_channel_irq_attach(ctx->dma_rx_chan, _uart_dma_rx_irq, (void *)ctx);

    qcc74x_irq_enable(((struct qcc74x_device_s *)(ctx->dma_tx_chan))->irq_num);
    qcc74x_irq_enable(((struct qcc74x_device_s *)(ctx->dma_rx_chan))->irq_num);
...
int uart_hw_reset(netbus_uart_ctx_t *ctx)
{
    ...
    dev = qcc74x_device_get_by_name(ctx->config->name);
    qcc74x_uart_deinit(dev);
...
ATTR_TCM_SECTION static inline void __uart_rx_cb(netbus_uart_ctx_t *pctx, struct qcc74x_device_s *pdev)
{
	int rxcnt = 0;
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;

    while (qcc74x_uart_rxavailable(pdev)) {
        rxcnt = qcc74x_uart_get(pdev, pctx->buffer, sizeof(pctx->buffer));
...
ATTR_TCM_SECTION static inline void __uart_tx_cb(netbus_uart_ctx_t *pctx, struct qcc74x_device_s *pdev)
{
	uint8_t ch;
	int ret;
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;

	while (qcc74x_uart_txready(pdev)) {
		ret = xStreamBufferReceiveFromISR(pctx->txbuf, &ch, 1, &xHigherPriorityTaskWoken);
	    if (ret > 0) {
	    	qcc74x_uart_putchar(pdev, ch);
	    } else {
	    	qcc74x_uart_txint_mask(pdev, true);
	    	break;
	    }
	}
...
ATTR_TCM_SECTION void uart_isr(int irq, void *arg)
{
	...
	struct qcc74x_device_s *pdev = qcc74x_device_get_by_name(pctx->config->name);
    uint32_t intstatus = qcc74x_uart_get_intstatus(pdev);

    if (intstatus & UART_INTSTS_RX_FIFO) {
        __uart_rx_cb(pctx, pdev);
    }
    if (intstatus & UART_INTSTS_RTO) {
        qcc74x_uart_int_clear(pdev, UART_INTCLR_RTO);
        __uart_rx_cb(pctx, pdev);
    }
...
int netbus_uart_init(netbus_uart_ctx_t *pctx, const netbus_uart_config_t *hwcfg, uint32_t txbuf_size, uint32_t rxbuf_size)
{
	...
	struct qcc74x_device_s *pdev = qcc74x_device_get_by_name(hwcfg->name);
    ...
    qcc74x_uart_txint_mask(pdev, true);
    qcc74x_uart_rxint_mask(pdev, false);
    qcc74x_irq_attach(pdev->irq_num, uart_isr, pctx);
    qcc74x_irq_enable(pdev->irq_num);
...
int netbus_uart_send(netbus_uart_ctx_t *pctx, const uint8_t *p_data, uint32_t len, uint32_t timeout)
{
	...
	while (remain_len) {
		...
		send_len += ret;
		remain_len -= ret;
		qcc74x_uart_txint_mask(pdev, false);
	}
...
{
	struct qcc74x_device_s *pdev = qcc74x_device_get_by_name(ctx->config->name);

    qcc74x_uart_txint_mask(pdev, true);
    qcc74x_uart_rxint_mask(pdev, true);
    qcc74x_irq_detach(pdev->irq_num);
    qcc74x_irq_disable(pdev->irq_num);
	qcc74x_uart_deinit(pdev);
