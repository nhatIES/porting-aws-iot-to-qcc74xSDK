...
#include <qcc74x_undef.h>
#include <qcc74x_undef_sdu.h>
...
static qcc74x_sdio_read_cb_t sd_read_callback = NULL;
...
int qcc74x_sdu_init(void)
{
    ...
    qcc74x_irq_attach(SDU_SOFT_RST_IRQn, sdu_soft_reset_interrupt_entry, NULL);
    CLIC->CLICINT[SDU_SOFT_RST_IRQn].ATTR |= 0x2;   /* rising edge trigger */
    qcc74x_irq_enable(SDU_SOFT_RST_IRQn);
...
int qcc74x_sdio_write_pbuf(void *env, uint16_t type, uint16_t subtype, struct pbuf *p, bool is_amsdu, void *cb, void *cb_arg)
{
    ...
    next = p->next;
    if (next) {
        HR_LOGE("QCC743 this should not happen, this is an A-AMSDU with two pbds\r\n");
        return QC_MSG_ERR_TIMEOUT;
    } else {
...
static void qcc74x_sdu_reinit(void)
{
    qcc74x_irq_disable(SDIO_IRQn);
...
static int sdio_software_reset(void)
{
    ...
    qcc74x_sdu_reinit();
    qc_net_wifi_trcver_reinit(&sdm->trcver);
