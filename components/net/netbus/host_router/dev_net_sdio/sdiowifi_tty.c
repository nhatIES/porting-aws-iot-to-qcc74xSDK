...
int sdiowifi_tty_send_bsr(sdiowifi_mgmr_t *sdm, uint16_t tty, bool initial, uint16_t incr)
{
    ...
    return qc_msg_send_frame_ext(&sdm->trcver, QC_MSG_TYPE_ETH_WIFI_FRAME, QC_MSG_ETH_WIFI_FRAME_SUBTYPE_EXT, &bsr, sizeof(bsr), NULL, 0);
}
...
ssize_t sdiowifi_tty_write(file_t *fp, const void *buf, size_t nbytes)
{
    ...
    while (nbytes > 0) {
        ...
        qc_msg_send_frame_ext(&sdm->trcver, QC_MSG_TYPE_ETH_WIFI_FRAME, QC_MSG_ETH_WIFI_FRAME_SUBTYPE_EXT,
