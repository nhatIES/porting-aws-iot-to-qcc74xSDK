...
static int qcc74x_send_simple_rnm_cmd_sync(struct qcc74x_eth_device *dev, uint16_t cmd_id)
{
    uint16_t tx_len;
    qcc74x_undef_sdio_msg_t *sdio_msg;
    rnm_base_msg_t cmd = {
        .cmd = cmd_id,
    };
    tx_len = sizeof(*sdio_msg) + sizeof(cmd);

    sdio_msg = (qcc74x_undef_sdio_msg_t *)dev->sdiocmd_buf;
    sdio_msg->type = QC_SDIO_MSG_TYPE_CMD;
    sdio_msg->len = tx_len;
    memcpy(sdio_msg->payload, &cmd, sizeof(cmd));

    if (qcc74x_sdio_write(dev, sdio_msg, tx_len))
        return -1;
    return 0;
}
...
int qcc74x_eth_get_mac_addr(struct qcc74x_eth_device *dev)
{
    ...
    for (i = 0; i < ETH_ALEN; ++i) {
        ret = qcc74x_read_reg(dev, QCC74x_WIFI_MAC_ADDR_OFFSET + i, &mac_addr[i]);
        if (ret) {
            break;
        }
    }
...
static int push_cmd(struct qcc74x_eth_device *dev, const void *payload, size_t len)
{
    struct sk_buff *skb;
    qcc74x_undef_sdio_msg_t *sdio_msg;
    ...
    sdio_msg = (qcc74x_undef_sdio_msg_t *)skb_put(skb, smsg_len);
    sdio_msg->type = QC_SDIO_MSG_TYPE_CMD;
...
int send_tty_frame(struct qcc74x_eth_device *dev, uint16_t tty_id, const void *data, size_t len)
{
    uint16_t tx_len;
    const uint16_t subtype = QC_MSG_ETH_WIFI_FRAME_SUBTYPE_EXT;
    qcc74x_undef_sdio_msg_t *msg = (qcc74x_undef_sdio_msg_t *)dev->sdiocmd_buf;
    ...
    msg->type = QC_SDIO_MSG_TYPE_ETH_FRAME;
    msg->len = tx_len;
    ...
    qcc74x_sdio_write(dev, dev->sdiocmd_buf, tx_len);
    return 0;
}

int send_hello(struct qcc74x_eth_device *dev)
{
    return qcc74x_send_simple_rnm_cmd_sync(dev, QC_CMD_HELLO);
}
...
// TODO: optimise me
int send_eth_frame(struct qcc74x_eth_device *dev, struct sk_buff *skb)
{
    uint16_t tx_len;
    void *cp_dst;
...
static void handle_eth_frame_rx(struct qcc74x_eth_device *dev, const uint16_t subtype, const void *eth_hdr, const void *frame_p1, const uint16_t frame_len1, const void *frame_p2, const uint16_t frame_len2)
{
    struct sk_buff *skb;
...
static void handle_ext_frame(struct qcc74x_eth_device *dev, const uint8_t *frame, const uint16_t frame_len)
{
    ...
    case SDIOWIFI_EXT_FRAME_TYPE_TTY:
        qcc74x_handle_ext_frame_tty(dev, frame, frame_len);
        break;
    default:
...
static void handle_eth_frame(struct qcc74x_eth_device *dev, const uint16_t subtype, const void *eth_hdr, const void *frame_p1, const uint16_t frame_len1, const void *frame_p2, const uint16_t frame_len2, uint16_t len_orig)
{
    switch (subtype) {
    case QC_MSG_ETH_WIFI_FRAME_SUBTYPE_STA_FROM_WIFI_RX:
        // fallthrough
    case QC_MSG_ETH_WIFI_FRAME_SUBTYPE_AP_FROM_WIFI_RX:
...
static int handle_amsdu_pdus(struct qcc74x_eth_device *dev, void *pdu1, void *pdu2)
{
    ...
    while (1) {
        ...
        // XXX type
        handle_eth_frame(dev, QC_MSG_ETH_WIFI_FRAME_SUBTYPE_STA_FROM_WIFI_RX, &eth_header, eth_frm_pld[0], eth_frm_pld_len[0], eth_frm_pld[1], eth_frm_pld_len[1], 0);
...
{
    struct qcc74x_eth_device *dev = (struct qcc74x_eth_device *)cb_arg;
    ...
    while (num_of_pdu > 0) {
        if (saved_pdu_used) {
            ...
            if (!msg->is_amsdu) {
                ...
                if (type == QC_SDIO_MSG_TYPE_ETH_FRAME) {
                ...
                } else if (type == QC_SDIO_MSG_TYPE_CMD) {
                    qcc74x_handle_cmd(dev, pld, pld_len);
                } else {
