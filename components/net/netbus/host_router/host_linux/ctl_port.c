...
static struct qcctl_dev ctl_dev;

static int nl_msg_out(struct sock *sock, const void *data, size_t len, u32 pid);
static int nl_bcast(struct qcctl_dev *cd, const void *data, size_t len);
...
static int parse_cmd(const u8 *payload, const u16 len)
{
    ...
    qcc74x_wifi_mode_t old_mode = QCC74x_MODE_MAX;
    int xfer_ret = -1;
    ...
    switch (cmd->cmd) {
        case QC_CMD_STA_CONNECT:
            if (!(xfer_ret = qcc74x_mode_xfer(&dev->mode, QCC74x_MODE_STA, &old_mode))) {
                // empty
            }
            qcc74x_change_eth_mac(dev);
            break;
        case QC_CMD_STA_DISCONNECT:
            if (!(xfer_ret = qcc74x_mode_xfer(&dev->mode, QCC74x_MODE_NONE, &old_mode))) {
                // empty
            }
            break;
        case QC_CMD_AP_START:
            if (!(xfer_ret = qcc74x_mode_xfer(&dev->mode, QCC74x_MODE_AP, &old_mode))) {
    ...
    if (!xfer_ret) {
        printk("mode %s->%s\n", qcc74x_mode_to_str(old_mode), qcc74x_mode_to_str(dev->mode));
    }
    mutex_unlock(&gl_dev.mutex);
...
int qcc74x_register_ctl_port()
{
    ...
    ctl_dev.nl_sock = netlink_kernel_create(&init_net, QCCTL_PROTO, &cfg);
    if (!ctl_dev.nl_sock) {
