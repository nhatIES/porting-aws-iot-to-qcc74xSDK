...
int parse_get_mac_rsp_msg(struct qcc74x_eth_device *dev, void *buf, int buf_len)
{
    ...
    if (rsp->hdr.cmd != QC_CMD_GET_MAC_ADDR || !(rsp->hdr.flags & RNM_MSG_FLAG_ACK)) {
        return -1;
    }
...
int push_host2device_msg(const uint8_t *payload, size_t len)
{
    ...
    ret = qcc74x_write_data_sync(dev, skb->data, skb->len, QCC74x_USB_TIMEOUT);
...
static void handle_eth_frame_rx(struct qcc74x_eth_device *dev, struct sk_buff *skb)
{
    skb->dev = dev->net;
...
int qcc74x_handle_rx_data(struct qcc74x_eth_device *dev, struct sk_buff *skb)
{
    ...
    if (type == USBWIFI_DATA_TYPE_CMD) {
        qcc74x_handle_cmd(dev, skb->data, skb->len);
        dev_kfree_skb_any(skb);
