...
#define QCCTL_PROTO NETLINK_USERSOCK
...
typedef struct {
    ...
    qcctl_wildcard_msg_handler_t msg_handler;
...
static int nl_msg_out(qcctl_env_t *env, const void *data, size_t len)
{
    int ret = 0;
    ...
    if (!nlh) {
        qcc74x_loge("no mem for netlink header\n");
        return ENOMEM;
    }
    ...
    if (sendmsg(env->sock, &msg, 0) < 0) {
        qcc74x_loge("sendmsg failed, %d, %s\n", errno, strerror(errno));
        ret = errno;
    }
    free(nlh);
...
static int nl_msg_recv(qcctl_env_t *env, size_t timeout_ms)
{
    int ret = 0;
    ...
    if (setsockopt(env->sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof tv)) {
        qcc74x_loge("setsockopt failed, %d, %s\n", errno, strerror(errno));
        ret = errno;
        goto exit;
    }
    ...
    if (ret < 0) {
        if (errno != EAGAIN)
            qcc74x_loge("recvmsg failed, %d, %s\n", errno, strerror(errno));
...
static int ctl_msg_send(qcctl_env_t *env, uint16_t type, const void *payload, size_t payload_len)
{
    int ret = 0;
...
int qcctl_msg_recv(qcctl_handle_t handle, void **rsp, size_t timeout_ms)
{
    ...
    qcctl_env_t *env = (qcctl_env_t *)handle;
    ...
    if (!memcmp(resp->magic, CTL_PORT_MSG_MAGIC, sizeof(resp->magic))) {
        ...
        qcc74x_loge("recved wrong resp msg magic, "
 ...
 */
static int ctl_msg_recv_type(qcctl_env_t *env, uint16_t rsp_type, void **rsp, size_t timeout_ms)
{
    int ret = 0;
    ctl_port_msg_hdr_t *resp = NULL;

    if ((ret = qcctl_msg_recv((qcctl_handle_t)env, (void *)&resp, timeout_ms)))
        goto out;
...
static int ctl_msg_send_expect_recv(qcctl_env_t *env, uint16_t type, const void *payload, size_t payload_len, uint16_t rsp_type, void **rsp, size_t timeout_ms)
{
    int ret;
...
static int create_session(qcctl_env_t *env)
{
    int ret;
...
static int delete_session(qcctl_env_t *env)
{
    int ret;
    ctl_port_msg_conn_rsp_t *rsp;
...
static wait_env_t *alloc_wait_env(qcctl_env_t *env, rnm_base_msg_t *msg)
{
    wait_env_t *ret = NULL;
    size_t i;
    ...
    if (i >= WAIT_SLOTS) {
        qcc74x_logw("wait slots are empty now, here are active slots, please check:\n");
        for (i = 0; i < WAIT_SLOTS; i++)
            qcc74x_logw("wait slot[%d]: cmd_id %d\n", i, env->wait_slots[i]->cmd_id);
        return ret;
    }
...
static void free_wait_env(qcctl_env_t *env, wait_env_t *wait)
{
    pthread_mutex_lock(&env->wait_slots_lock);
...
int qcctl_rnm_msg_expect_simple(qcctl_handle_t handle, void *msg, size_t msg_len, uint16_t *status, size_t timeout_ms)
{
    ...
    if ((ret = qcctl_rnm_msg_expect(handle, msg, msg_len, (void *)&rsp, timeout_ms)))
        return ret;
...
/* this api will deprecation, use qcctl_rnm_user_send_extension replace */
...
static int qcctl_send_hello(qcctl_env_t *env)
{
    rnm_base_msg_t msg;

    memset(&msg, 0, sizeof(msg));
    msg.cmd = QC_CMD_HELLO;
...
{
    int ret = 0;
    qcctl_env_t *env = NULL;
    int sock = -1;
    struct sockaddr_nl src_addr;
    struct nlmsghdr *rcv_nlh = NULL;

    if (!(env = calloc(1, sizeof(qcctl_env_t)))) {
        qcc74x_loge("no mem for qcctl env\n");
        ret = ENOMEM;
        goto error;
    }

    if (!(rcv_nlh = malloc(NLMSG_SPACE(NL_RCV_MAX_PAYLOAD)))) {
        qcc74x_loge("no mem for qcctl env recv buffer\n");
        ret = ENOMEM;
        goto error;
    }

    sock = socket(PF_NETLINK, SOCK_RAW, QCCTL_PROTO);
    if (sock < 0) {
        qcc74x_loge("failed to create netlink socket, %d, %s\n",
                errno, strerror(errno));
        ret = errno;
        goto error;
    }
    ...
    if (bind(sock, (struct sockaddr *)&src_addr, sizeof(src_addr)) < 0) {
        qcc74x_loge("failed to bind netlink socket, %d, %s\n",
                errno, strerror(errno));
        ret = errno;
        goto error;
    }
    ...
    if ((ret = create_session(env))) {
        qcc74x_loge("failed to create new session, %d\n", ret);
        goto error;
    }
    ...
    *handle = env;
    if (daemon && qcctl_send_hello(env)) {
        qcc74x_loge("qcctl failed to send hello\r\n");
    }
    return 0;
...
{
    return qcctl_init2(handle, false);
}
