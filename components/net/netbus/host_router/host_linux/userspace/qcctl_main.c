...
static qcctl_handle_t ctl = NULL;

void usage(const char *prog)
{
    fprintf(stderr, "qcctl version " QCCTL_VERSION "\n\n"
                    "Usage: %s <cmd> [cmd arg]\n"
                    "          <cmd>:\n"
...
int wifi_scan()
{
    ...
    return qcctl_rnm_msg_expect_simple(ctl, (void *)&cmd, sizeof(cmd), NULL, 100);
}
...
int wifi_scan_results()
{
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.cmd = QC_CMD_SCAN_RESULTS;

    if ((ret = qcctl_rnm_msg_expect(ctl, &cmd, sizeof(cmd), (void *)&rsp, 100)))
        return ret;
    for (i = 0; i < rsp->num; ++i) {
        struct qc_wifi_scan_record *r = &rsp->records[i];
        printf("BSSID " MAC_FMT ", channel %u, rssi %d, auth %s, cipher %s, SSID %s\n",
                ARR_ELE_6(r->bssid), r->channel, r->rssi,
                qcctl_auth_to_str(r->auth_mode), qcctl_cipher_to_str(r->cipher), r->ssid);
    }
    free(rsp);
...
int connect_ap(const char *ssid, const int ssid_len, const char *password, const int pwd_len)
{
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.hdr.cmd = QC_CMD_STA_CONNECT;
    cmd.ssid_len = ssid_len;
    memcpy(cmd.ssid, ssid, ssid_len);
    if (password) {
        memcpy(cmd.password, password, pwd_len);
    }
    if ((ret = qcctl_rnm_msg_expect_simple(ctl, (void *)&cmd, sizeof(cmd), &status, 100))) {
        qcc74x_loge("failed to connect to AP, %d\n", ret);
        return ret;
    }
...
int disconnect_ap()
{
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.cmd = QC_CMD_STA_DISCONNECT;

    if ((ret = qcctl_rnm_msg_expect_simple(ctl, (void *)&cmd, sizeof(cmd), &status, 100))) {
        qcc74x_loge("failed to disconnect from AP, %d\n", ret);
        return ret;
    }
    return 0;
}
...
int stop_ap()
{
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.cmd = QC_CMD_AP_STOP;

    return qcctl_rnm_msg_expect_simple(ctl, (void *)&cmd, sizeof(cmd), &status, 100);
}
...
int set_auto_connect(const char *opt)
{
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.hdr.cmd = QC_CMD_STA_SET_AUTO_RECONNECT;
    if (opt[0] == '1') {
        cmd.en = 1;
    } else {
        cmd.en = 0;
...
int qcctl_handle_one_rnm(qcctl_handle_t handle, rnm_base_msg_t *msg,
        void *data, int len);
...
int demo_user_ext(void)
{
    ...
    if ((ret = qcctl_rnm_user_ext_send(ctl, m, sizeof(m))))
        return ret;
    usleep(100 * 1000);
...
int user_ext_cmd_example(uint8_t is_response)
{
    ...
    qcctl_register_wildcard_msg_handler(ctl, wildcard_msg_handler, ctl);
    if (is_response) {
        if ((ret = qcctl_rnm_user_send_extension(ctl, m, sizeof(m), QC_CMD_USER_EXT_RSP))) {
            return ret;
        }
    } else {
        if ((ret = qcctl_rnm_user_send_extension(ctl, m, sizeof(m), QC_CMD_USER_EXT_NO_RSP))) {
            return ret;
        }
    }
...
int get_link_status()
{
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.cmd = QC_CMD_STA_GET_LINK_STATUS;

    if ((ret = qcctl_rnm_msg_expect(ctl, &cmd, sizeof(cmd), (void *)&rsp, 100)))
        return ret;
    if (rsp->record.link_status == QC_WIFI_LINK_STATUS_UP) {
        printf("UP RSSI %d\n", rsp->record.rssi);
    } else {
        printf("DOWN\n");
    }
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stdout, "qcctl version %s\n", QCCTL_VERSION);
    atexit(exit_handler);
    ...
    if ((ret = qcctl_init(&ctl))) {
        qcc74x_loge("qcctl init failed, %d\n", ret);
        ret = 1;
        goto ret;
    }

    qcctl_register_wildcard_msg_handler(ctl, wildcard_msg_handler, NULL);
    cmd = argv[1];
