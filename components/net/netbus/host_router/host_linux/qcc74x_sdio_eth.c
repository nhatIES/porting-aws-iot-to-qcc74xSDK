...
#include "qcc74x_sdio.h"
#include "sdio_msg_handlers.h"
#include "ctl_port.h"
#include "tty.h"
#include "version.h"

static void send_cmds(struct qcc74x_eth_device *dev);
...
static const struct sdio_device_id qcc74x_sdio_ids[] = {
    { SDIO_DEVICE(SDIO_VENDOR_ID, SD_DEVICE_ID) },
    { 0 }
};
MODULE_DEVICE_TABLE(sdio, qcc74x_sdio_ids);

void qcc74x_sdio_deinit(struct qcc74x_eth_device *dev)
{
...
static int qcc74x_rx_process(struct qcc74x_eth_device *dev, struct sk_buff *skb, u32 port);
int rx_task(void *data)
{
    ...
    dev = (struct qcc74x_eth_device*)data;
    ...
    while(1){
        ...
        qcc74x_read_reg(dev, dev->reg->rd_bitmap_l, &rd_bitmap_l);
        qcc74x_read_reg(dev, dev->reg->rd_bitmap_u, &rd_bitmap_u);

        bitmap = rd_bitmap_l;
        bitmap |= rd_bitmap_u << 8;

        if (bitmap == 1) {
            qcc74x_rx_process(dev, NULL, 0x10000);
        }
...
static int qcc74x_sdio_get_ready(struct qcc74x_eth_device *dev)
{
    u8 val;
    int ret;
    int retry = 0;
...
static int qcc74x_sdio_init(struct qcc74x_eth_device *dev)
{
    int ret = 0;
    u8 sdio_ireg;

    dev->reg = &qcc74x_reg_sd606;
    dev->max_ports = 1;
    ...
    /* 4. set block size*/
    sdio_set_block_size(dev->func, QCC74x_SDIO_BLOCK_SIZE);

    /* 5. request irq */
#if !USE_POLLING_RX
    ret = sdio_claim_irq(dev->func, qcc74x_irq_hdlr);
    if (ret) {
        pr_err("sdio_claim_irq failed: ret=%d\n", ret);
        goto err_out;
    }
#else
    rx_polling_task = kthread_create(rx_task, dev, "qcc74x_rx");
...
static void qcc74x_get_interrupt_status(struct qcc74x_eth_device *dev)
{
    u8 sdio_ireg;

    qcc74x_read_reg(dev, dev->reg->host_int_status_reg, &sdio_ireg);
    if(sdio_ireg) {
...
int qcc74x_sdio_write(struct qcc74x_eth_device *dev, const void *data_ptr, const uint16_t data_len)
{
    ...
    //printk("===== qcc74x_sdio_write dnld port:%d, bitmap:%X\r\n", dev->curr_wr_port, dev->mp_wr_bitmap);
    start_tick = jiffies;
    while ((dev->mp_wr_bitmap & (1 << dev->curr_wr_port)) == 0) {
        ...
        qcc74x_read_reg(dev, dev->reg->wr_bitmap_l, &wr_bitmap_l);
        qcc74x_read_reg(dev, dev->reg->wr_bitmap_u, &wr_bitmap_u);
    ...
    qcc74x_write_data_sync(dev, ptx_data, 2048, 0x10000 + dev->curr_wr_port);
...
int qcc74x_sdio_read(struct qcc74x_eth_device *dev, void **data)
{
    ...
    //printk("===== qcc74x_sdio_read upld bitmap:%X\r\n", bitmap);
    if (qcc74x_read_data_sync(dev, rx_buf, 2048, 0x10000 + dev->my_curr_rd_port))
        return -1;
...
int qcc74x_sdio_read_cb_register(struct qcc74x_eth_device *dev, qcc74x_sdio_read_cb_t cb, void *cb_arg)
{
    if (cb == NULL) {
        return -1;
    }
...
static int rx_data(struct qcc74x_eth_device *dev, u8 *rx_buf, u32 port, u32 len)
{
    if (qcc74x_read_data_sync(dev, rx_buf, len, port)) {
        return -1;
    }
    return 0;
}
...
static int qcc74x_rx_noprocess(struct qcc74x_eth_device *dev, struct sk_buff *skb, u32 port)
{
    u8 *prx_data;
    struct xfr_ctx *xfr;
...
static int qcc74x_rx_process_agg(struct qcc74x_eth_device *dev, u16 bitmap)
{
    uint16_t len;
    u8 *prx_data;
    struct xfr_ctx *xfr;
...
static int qcc74x_upload_hdl(struct qcc74x_eth_device *dev)
{
    u8 rd_bitmap_l = 0;
    u8 rd_bitmap_u = 0;
    u32 bitmap;
    ...
    //printk("qcc74x_upload_hdl: bitmap %x, cur_rd_port %u\n", bitmap, dev->my_curr_rd_port);
    if (!bitmap) {
        u32 fake_rd_port;
        ...
        qcc74x_rx_noprocess(dev, NULL, fake_rd_port);
        return 0;
    }

    do {
        if (loop_cnt > 1) {
            /* printk("qcc74x_upload_hdl: bitmap %x\n", bitmap); */
            /* printk("Process loop %d\n", loop_cnt); */
            if (loop_cnt > 16)
                break;
        }
        qcc74x_rx_process_agg(dev, bitmap);
        qcc74x_read_reg(dev, dev->reg->rd_bitmap_l, &rd_bitmap_l);
        qcc74x_read_reg(dev, dev->reg->rd_bitmap_u, &rd_bitmap_u);
        bitmap = rd_bitmap_l;
        bitmap |= rd_bitmap_u << 8;
...
#if 0
    if (bitmap & (1 << dev->my_curr_rd_port)) {
        if (qcc74x_rx_process(dev, NULL, 0x10000 + dev->my_curr_rd_port)) {
            printk("qcc74x_rx_process error\n");
            return -1;
        }
    }
#endif
    ...
    do {
        ...
        for (i = 0; i < 16; ++i) {
            if (bitmap & (1 << i)) {
                if (qcc74x_rx_process(dev, NULL, 0x10000 + i)) {
                    printk("qcc74x_rx_process error\n");
                    return -1;
                }
            }
...
static inline int qcc74x_tx_credit_available(struct qcc74x_eth_device *dev)
{
    return (s8)(dev->seq_clamp - dev->snd_nxt) > 0;
}

#define QCC74x_TX_CREDIT_TIMEOUT_MS 2500

static bool qcc74x_eth_tx_data_process(struct qcc74x_eth_device *dev)
{
    struct sk_buff *skb;
    size_t txed = 0;
    bool restart = false;
    ...
    while ((skb = skb_dequeue(&dev->sk_list)) != NULL) {
        check_point = jiffies + msecs_to_jiffies(QCC74x_TX_CREDIT_TIMEOUT_MS);
        while (1) {
            if (!qcc74x_card_ok()) {
                dev_kfree_skb_any(skb);
                break;
            ...
            if (qcc74x_tx_credit_available(dev)) {
                qcc74x_tx_push(dev, skb);
                dev->snd_nxt++;
                break;
            } else {
                qcc74x_read_reg(dev, FREE_TX_PBUF_CNT_OFFSET, &dev->seq_clamp);
                if (!qcc74x_tx_credit_available(dev)) {
                    ...
                    if (time_after(jiffies, check_point)) {
                        ...
                        pr_warn("the ethernet frame is dropped because we "
                                ...
                                QCC74x_TX_CREDIT_TIMEOUT_MS, dev->snd_nxt,
                                dev->seq_clamp);
                        break;
                    }
                }
...
static void send_tty(struct qcc74x_eth_device *dev)
{
    struct sk_buff *skb;

    while ((skb = skb_dequeue(&dev->tty_msg_list)) != NULL) {
        struct qcc74x_tty_queued_data *qd;
        u16 tty_id;
        unsigned skb_len;
        int avail_len;
        unsigned tx_len;
        ...
        qd = (struct qcc74x_tty_queued_data *)skb->cb;
        tty_id = qd->tty_id;
...
{
    struct qcc74x_eth_device *dev = container_of(work, struct qcc74x_eth_device, tx_work);

    spin_lock_irq(&dev->tx_proc_lock);
    if (dev->qcc74x_tx_processing) {
        printk("qcc74x_main_process: tx_more_task_flag = true\n");
        dev->tx_more_task_flag = true;
        spin_unlock_irq(&dev->tx_proc_lock);
        goto exit_main_proc;
    } else {
        dev->qcc74x_tx_processing = true;
        spin_unlock_irq(&dev->tx_proc_lock);
    }
...
process_start:
    send_cmds(dev);
    send_tty(dev);
    if (qcc74x_eth_tx_data_process(dev)) {
        goto process_start;
    }
    ...
    dev->qcc74x_tx_processing = false;
    spin_unlock_irq(&dev->tx_proc_lock);
...
static void send_cmds(struct qcc74x_eth_device *dev)
{
    ...
    while ((skb = skb_dequeue(&dev->cmd_list))) {
        ...
        qcc74x_sdio_write(dev, skb->data, skb->len);
        dev_kfree_skb_any(skb);
    }
}
...
void main_wq_hdlr(struct work_struct *work)
{
    ...
    if (dev->qcc74x_processing) {
        printk("qcc74x_main_process: more_task_flag = true\n");
...
process_start:
    ...
    if (dev->check_card_flag) {
        ...
        if (time_after(jiffies, dev->last_ping_recv_time + msecs_to_jiffies(CARD_DEAD_THRESHOLD_MS))) {
            ...
            gl_dev.status |= QCC74x_DEVICE_STATUS_CARD_DEAD;
...
void qcc74x_irq_hdlr(struct sdio_func *func)
{
    struct qcc74x_eth_device *dev;
    ...
    qcc74x_get_interrupt_status(dev);
...
static int init_xfr_ctx(struct qcc74x_eth_device *dev)
{
...
static void deinit_xfr_ctx(struct qcc74x_eth_device *dev)
{
...
{
    struct qcc74x_eth_device *dev = from_timer(dev, t, check_card_timer);
...
static int qcc74x_stats_proc_open(struct inode *inode, struct file *file)
{
...
static ssize_t qcc74x_stats_proc_read(struct file *file, char __user *buffer,
                                size_t count, loff_t *ppos)
{
    char *str;
    size_t len;
    ssize_t ret = 0;
    struct qcc74x_eth_device *dev = file->private_data;
...
static const struct proc_ops stats_proc_ops = {
    .proc_open = qcc74x_stats_proc_open,
    .proc_read = qcc74x_stats_proc_read,
};

#define QCC74x_PROC_STATS_NAME    "stats"

extern struct proc_dir_entry *qcc74x_proc_dir;
/* QCC74x_PROCFS_ENABLE */
#endif

static int qcc74x_query_version(struct qcc74x_eth_device *dev)
{
    uint16_t tx_len;
    uint8_t buf[256];
    rnm_base_msg_t *hdr;
    struct qcc74x_version_query *query;

    tx_len = sizeof(rnm_base_msg_t) + sizeof(struct qcc74x_version_query);
    hdr = (rnm_base_msg_t *)buf;
    hdr->cmd = QC_CMD_GET_DEV_VERSION;
    hdr->flags = 0;
    hdr->msg_id = 0;
    hdr->status = 0;
    hdr->session_id = 0;
    ...
    query->minor_version = QCC74x_WNET_MINOR_VERSION;
    query->major_version = QCC74x_WNET_MAJOR_VERSION;
    memset(query->reserved, 0, sizeof(query->reserved));
    /* call this api instead of qcc74x_sdio_write to avoid race condition */
    return push_host2device_msg(buf, tx_len);
}

static int qcc74x_do_handshake(struct qcc74x_eth_device *dev)
{
    int err;
    int tryouts = 3;
...
static int qcc74x_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
    int retval = 0;
    ...
    dev->txworkqueue = alloc_workqueue("QCC74x_TX_WORK_QUEUE", WQ_HIGHPRI | WQ_MEM_RECLAIM | WQ_UNBOUND, 1);
    if (!dev->txworkqueue) {
        printk("creat workqueue failed\n");
        retval = -ENOMEM;
        goto err_create_wkq;
    ...
    qcc74x_sdio_read_cb_register(dev, sdio_read_callback, dev);
    /*init sdio device*/
    if ((retval = qcc74x_sdio_init(dev))) {
        goto err_sdio_init;
    }

    retval = qcc74x_eth_get_mac_addr(dev);
    if (retval)
    ...
    mutex_lock(&gl_dev.mutex);
    gl_dev.status &= ~QCC74x_DEVICE_STATUS_CARD_REMOVING;
#if CARD_DEAD_CHECK
    gl_dev.status &= ~QCC74x_DEVICE_STATUS_CARD_DEAD;
#endif
    gl_dev.status |= QCC74x_DEVICE_STATUS_CARD_PRESENT;
...
#ifdef QCC74x_PROCFS_ENABLE
    proc_create_data(QCC74x_PROC_STATS_NAME, S_IFREG | 0644, qcc74x_proc_dir,
                     &stats_proc_ops, dev);
#endif
...
static void qcc74x_sdio_remove(struct sdio_func *func)
{
    struct qcc74x_eth_device *dev;

#ifdef QCC74x_PROCFS_ENABLE
    remove_proc_entry(QCC74x_PROC_STATS_NAME, qcc74x_proc_dir);
#endif
    mutex_lock(&gl_dev.mutex);
    dev = sdio_get_drvdata(func);
...
static struct sdio_driver qcc74x_sdio_drv = {
    .name     = "qcc74x_eth",
    .id_table = qcc74x_sdio_ids,
    .probe    = qcc74x_sdio_probe,
    .remove   = qcc74x_sdio_remove
};

int qcc74x_register_eth_drv(void)
{
    return sdio_register_driver(&qcc74x_sdio_drv);
}

void qcc74x_unregister_eth_drv(void)
{
    sdio_unregister_driver(&qcc74x_sdio_drv);
}
