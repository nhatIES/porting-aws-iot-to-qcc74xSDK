...
void qcc74x_get_rd_len(struct qcc74x_eth_device *dev, u32 reg_l, u32 reg_u, u32 *len)
{
    u8 rd_len_l = 0;
    u8 rd_len_u = 0;

    qcc74x_read_reg(dev, reg_l, &rd_len_l);
    qcc74x_read_reg(dev, reg_u, &rd_len_u);
 ...
 */
int qcc74x_get_rd_port(struct qcc74x_eth_device *dev, u32 *port)
{
 ...
 */
int qcc74x_get_wr_port(struct qcc74x_eth_device *dev, u32 *port)
{
...
#define QCC74x_SDIO_WRITE_MAX_TRYOUTS    3
/*
 * This function writes multiple data into SDIO card memory.
 *
 * This does not work in suspended mode.
 */
int qcc74x_write_data_sync(struct qcc74x_eth_device *dev, u8 *buffer, u32 pkt_len, u32 port)
{
    int i, ret;
    u8 blk_mode = (pkt_len < QCC74x_SDIO_BLOCK_SIZE) ? BYTE_MODE : BLOCK_MODE;
    u32 blk_size = (blk_mode == BLOCK_MODE) ? QCC74x_SDIO_BLOCK_SIZE : 1;
    u32 blk_cnt = (blk_mode == BLOCK_MODE) ? ((pkt_len + QCC74x_SDIO_BLOCK_SIZE -1)/QCC74x_SDIO_BLOCK_SIZE) : pkt_len;

    u32 ioport = (port & QCC74x_SDIO_IO_PORT_MASK);
    ...
    do {
        ...
        if (ret < 0) {
            ...
            if (i < QCC74x_SDIO_WRITE_MAX_TRYOUTS) {
 ...
 */
int qcc74x_read_data_sync_claim0(struct qcc74x_eth_device *dev, u8 *buffer, u32 len, u32 port)
{
    int ret;
    u8 blk_mode = (len < QCC74x_SDIO_BLOCK_SIZE) ? BYTE_MODE : BLOCK_MODE;
    u32 blk_size = (blk_mode == BLOCK_MODE) ? QCC74x_SDIO_BLOCK_SIZE : 1;
    u32 blk_cnt = (blk_mode == BLOCK_MODE) ? ((len + QCC74x_SDIO_BLOCK_SIZE -1)/QCC74x_SDIO_BLOCK_SIZE) : len;
    u32 ioport = (port & QCC74x_SDIO_IO_PORT_MASK);
 ...
 * This function reads multiple data from SDIO card memory.
 */
int qcc74x_read_data_sync(struct qcc74x_eth_device *dev, u8 *buffer, u32 len, u32 port)
{
    int ret;
    u8 blk_mode = (len < QCC74x_SDIO_BLOCK_SIZE) ? BYTE_MODE : BLOCK_MODE;
    u32 blk_size = (blk_mode == BLOCK_MODE) ? QCC74x_SDIO_BLOCK_SIZE : 1;
 ...
 */
int qcc74x_read_reg(struct qcc74x_eth_device *dev, u32 reg, u8 *data)
{
    int ret = -1;
    u8 val;
 ...
 */
int qcc74x_write_reg(struct qcc74x_eth_device *dev, u32 reg, u8 data)
{
    int ret;
