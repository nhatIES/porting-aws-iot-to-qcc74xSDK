...
void rnms_notify_sta_ip_addr(rnms_t *rnm)
{
    ...
    msg.hdr.cmd = QC_CMD_STA_IP_UPDATE_IND;
...
#else
    qcc74x_wifi_mac_addr_get(resp->sta_mac);
    qcc74x_wifi_mac_addr_get(resp->ap_mac);
#endif
}
...
static chr_t handle_scan_results(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    cnt_limit = (msg_len_limit - sizeof(rnm_scan_ind_msg_t)) / sizeof(struct qc_wifi_scan_record);
    cnt_total = wifi_mgmr_sta_scanlist_nums_get();
    if (cnt_total > cnt_limit) {
        cnt_total = cnt_limit;
    }
    ind_msg_len = sizeof(rnm_scan_ind_msg_t) + cnt_total * sizeof(struct qc_wifi_scan_record);
...
#else
// XXX not complete
static void fill_sta_status(struct qc_wifi_ap_record *record)
{
    int value_int;
    ...
    if (0 == wifi_mgmr_state_get(&value_int)) {
        if (WIFI_STATE_CONNECTED_IP_GOT == value_int ||
            ...
            record->link_status = QC_WIFI_LINK_STATUS_UP;
        } else {
            record->link_status = QC_WIFI_LINK_STATUS_DOWN;
        }
    } else {
        record->link_status = QC_WIFI_LINK_STATUS_UNKNOWN;
    }
...
static chr_t handle_sta_get_link_status(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    if (wifi_mgmr_sta_state_get()) {
        ...
        rsp.record.link_status = QC_WIFI_LINK_STATUS_UP;
        rsp.record.rssi = rssi;
        rsp.record.channel = channel;
    } else {
        rsp.record.link_status = QC_WIFI_LINK_STATUS_DOWN;
    }
#else
...
#ifdef CFG_QCC743
#else
static void sniffer_cb(void *env, void *pkt_wrap, qcc74x_rx_info_t *info)
{
    rnms_t *rnm = env;
...
// clang-format off
static const struct cmd_handler cmd_handler_table[QC_CMD_MAX] = {
    [QC_CMD_REBOOT]       = {handle_reboot},
    [QC_CMD_RESET]        = {handle_reset},
    [QC_CMD_HELLO]        = {handle_hello},
    [QC_CMD_PING]         = {handle_ping},
    [QC_CMD_GET_MAC_ADDR] = {handle_get_mac_addr},
    [QC_CMD_SCAN]         = {handle_scan},
    [QC_CMD_SCAN_RESULTS] = {handle_scan_results},
    // STA
    [QC_CMD_STA_CONNECT]            = {handle_sta_connect},
    [QC_CMD_STA_DISCONNECT]         = {handle_sta_disconnect},
    [QC_CMD_STA_SET_AUTO_RECONNECT] = {handle_sta_set_auto_reconnect},
    [QC_CMD_STA_GET_LINK_STATUS]    = {handle_sta_get_link_status},
    // AP
    [QC_CMD_AP_START]        = {handle_ap_start},
    [QC_CMD_AP_STOP]         = {handle_ap_stop},
    [QC_CMD_AP_GET_STA_LIST] = {handle_ap_get_sta_list},
    // Monitor
    [QC_CMD_MONITOR_START]       = {handle_monitor_start},
    [QC_CMD_MONITOR_STOP]        = {handle_monitor_stop},
    [QC_CMD_MONITOR_SET_CHANNEL] = {handle_montior_set_channel},
    [QC_CMD_MONITOR_GET_CHANNEL] = {handle_montior_get_channel},

    [QC_CMD_SET_LPM_MODE] = {handle_set_lpm_mode},

    [QC_CMD_GET_DEV_VERSION] = {handle_get_dev_version},
    [QC_CMD_OTA]             = {handle_ota},

    [QC_CMD_USER_EXT] = {handle_user_ext},
    [QC_CMD_USER_EXT_RSP] = {handle_user_ext_respones},
    [QC_CMD_USER_EXT_NO_RSP] = {handle_user_ext_no_respones},
};
// clang-format on

static void check_transparent_txed(rnms_t *rnm, rnm_msg_t *msg, int output_ret)
{
    if (!(output_ret == RNM_OUTPUT_FN_TIMEOUT && msg->cmd.cmd == QC_CMD_PING &&
                rnm->heartbeat_timer)) {
        return;
    }
...
void rnms_msg_process(rnms_t *rnm, rnm_msg_t *msg)
{
    ...
    if (cmd_id < QC_CMD_MAX && cmd_handler_table[cmd_id].handler) {
