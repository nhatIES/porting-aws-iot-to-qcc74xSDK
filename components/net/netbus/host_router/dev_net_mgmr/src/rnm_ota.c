...
static void fill_ota_status(rnms_t *rnm, rnm_msg_t *cmd, rnm_ota_msg_t *msg, uint16_t status_type)
{
    ...
    msg->msg_type = QCC74x_OTA_MSG_STATUS;
    msg->status.status_type = status_type;
}

static void reply_error(rnms_t *rnm, rnm_msg_t *cmd, qcc74x_ota_error_code_t code)
{
    rnm_ota_msg_t msg;

    fill_ota_status(rnm, cmd, &msg, QCC74x_OTA_ERROR);
...
#define MY_ERROR_OUT(e, r, p) error = e; ret = r; goto p
    int ret;
    qcc74x_ota_error_code_t error;
    rnm_ota_msg_t msg;
    rnm_debug("handle ota start\r\n");

    if (rnm->ota) {
        HR_LOGE("ota in progress?\r\n");
        MY_ERROR_OUT(QCC74x_OTA_ERROR_STATUS_INCORRECT, -1, error1);
    }
    rnm->ota = ota_ctx_alloc();
    if (!rnm->ota) {
        HR_LOGE("ota malloc failed\r\n");
        MY_ERROR_OUT(QCC74x_OTA_ERROR_MISC, -1, error1);
    }
    rnm_ota_t *octx = (rnm_ota_t *)rnm->ota;

    ret = qcc74x_mtd_open(RNM_MTD_PARTITION_NAME_FW_DEFAULT, &octx->mtd_hdl, RNM_MTD_OPEN_FLAG_BACKUP);
    if (ret) {
    ...
    if (hal_boot2_get_active_entries(BOOT2_PARTITION_TYPE_FW, &octx->ptEntry)) {
        ...
        MY_ERROR_OUT(QCC74x_OTA_ERROR_MISC, -1, error2);
    }
    ...
    qcc74x_mtd_erase_all(octx->mtd_hdl);
    rnm_warning("done\r\n");

    fill_ota_status(rnm, cmd, &msg, QCC74x_OTA_START_ACK);

    rnms_msg_output(rnm, &msg, sizeof(msg));
    return 0;
error2:
    qcc74x_mtd_close(octx->mtd_hdl);
error1:
...
static int handle_abort(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    fill_ota_status(rnm, cmd, &msg, QCC74x_OTA_ABORT_ACK);
...
#define MY_ERROR_OUT(e, r, p) error = e; ret = r; goto p
    ...
    if (!((part_offset == 0 && octx->offset_written == 0) ||
        ...
        MY_ERROR_OUT(QCC74x_OTA_ERROR_PART_MISSING, -1, error1);
    }
    if (!(part_size <= RNM_OTA_PART_SIZE)) {
        HR_LOGE("part size too large %lu\r\n", part_size);
        MY_ERROR_OUT(QCC74x_OTA_ERROR_SIZE_NOT_EXPECTED, -1, error1);
    }
    ...
    if (part_offset == 0) {
        ...
        if (_check_ota_header(ota_header, &hdr_adv_bin_size, &use_xz)) {
            MY_ERROR_OUT(QCC74x_OTA_ERROR_MALFORMED_HEADER, -1, error1);
        }
        ...
        if (!(hdr_adv_bin_size > 0 && hdr_adv_bin_size <= octx->tgt_part_size)) {
            MY_ERROR_OUT(QCC74x_OTA_ERROR_HEADER_ADV_LEN_TOO_BIG, -1, error1);
        }
    ...
    qcc74x_mtd_write(octx->mtd_hdl, octx->offset_written, part_size, pb);
    qcc74x_mtd_read(octx->mtd_hdl, octx->offset_written, part_size, pb);
    ...
    if (is_last_part) {
        ...
        if (memcmp(octx->sha256_this, octx->sha256_that, sizeof(octx->sha256_this))) {
            ...
            MY_ERROR_OUT(QCC74x_OTA_ERROR_CKSUM_MISMATCH, -1, error1);
        } else {
    ...
    if (is_last_part) {
        ack_msg.status.status_type = QCC74x_OTA_FIN_ACK;
    } else {
        ack_msg.status.status_type = QCC74x_OTA_PROGRAM_ACK;
...
static int handle_commit(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    fill_ota_status(rnm, cmd, &msg, QCC74x_OTA_COMMIT_ACK);
...
void rnms_handle_ota(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    switch (msg->msg_type) {
    case QCC74x_OTA_MSG_START:
        handle_start(rnm, cmd);
        break;
    case QCC74x_OTA_MSG_ABORT:
        handle_abort(rnm, cmd);
        break;
    case QCC74x_OTA_MSG_PROGRAM_PART:
        handle_program_part(rnm, cmd);
        break;
    case QCC74x_OTA_MSG_COMMIT:
