    ...
    while (1) {
        ...
        if (0 == sdiowifi_queue_read(rnm->rnm_event_queue, &code, SDIOWIFI_WAIT_FOREVER, 0)) {
            if (code == CODE_WIFI_ON_CONNECTED) {
                rnms_send_simple_async_msg(rnm, QC_CMD_STA_CONNECTED_IND);
            } else if (code == CODE_WIFI_ON_DISCONNECT) {
                rnm->have_valid_sta_ip = false;
                rnms_send_simple_async_msg(rnm, QC_CMD_STA_DISCONNECTED_IND);
            } else if (code == CODE_WIFI_ON_AP_STARTED) {
                rnms_send_simple_async_msg(rnm, QC_CMD_AP_STARTED_IND);
            } else if (code == CODE_WIFI_ON_AP_STOPPED) {
                rnms_send_simple_async_msg(rnm, QC_CMD_AP_STOPPED_IND);
...
static void heartbeat_timer_cb(sdiowifi_timer_handle_t timer, void *arg)
{
    ...
    rnms_send_simple_async_msg_timeout(rnm, QC_CMD_PING, 0);
}
...
int rnms_send_simple_async_msg_timeout(rnms_t *rnm, qc_cmd_t cmd, int timeout_ms)
{
    rnm_msg_t msg;
...
int rnms_send_simple_async_msg(rnms_t *rnm, qc_cmd_t cmd)
{
