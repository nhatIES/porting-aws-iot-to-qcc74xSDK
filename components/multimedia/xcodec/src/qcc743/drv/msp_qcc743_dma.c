...
static void DMA_IntHandler(DMA_ID_Type dmaId)
{
    ...
    for (ch = 0; ch < DMA_CH_MAX; ch++) {
        tmpVal = QCC74x_RD_REG(DMAChs, DMA_INTTCSTATUS);

        if ((QCC74x_GET_REG_BITS_VAL(tmpVal, DMA_INTTCSTATUS) & (1 << ch)) != 0) {
            /* Clear interrupt */
            tmpVal = QCC74x_RD_REG(DMAChs, DMA_INTTCCLEAR);
            intClr = QCC74x_GET_REG_BITS_VAL(tmpVal, DMA_INTTCCLEAR);
            intClr |= (1 << ch);
            tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_INTTCCLEAR, intClr);
            QCC74x_WR_REG(DMAChs, DMA_INTTCCLEAR, tmpVal);
    ...
    for (ch = 0; ch < DMA_CH_MAX; ch++) {
        tmpVal = QCC74x_RD_REG(DMAChs, DMA_INTERRORSTATUS);

        if ((QCC74x_GET_REG_BITS_VAL(tmpVal, DMA_INTERRORSTATUS) & (1 << ch)) != 0) {
            /*Clear interrupt */
            tmpVal = QCC74x_RD_REG(DMAChs, DMA_INTERRCLR);
            intClr = QCC74x_GET_REG_BITS_VAL(tmpVal, DMA_INTERRCLR);
            intClr |= (1 << ch);
            tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_INTERRCLR, intClr);
            QCC74x_WR_REG(DMAChs, DMA_INTERRCLR, tmpVal);
...
void msp_DMA_Enable(DMA_ID_Type dmaId)
{
    ...
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_TOP_CONFIG);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, DMA_E);
    QCC74x_WR_REG(DMAChs, DMA_TOP_CONFIG, tmpVal);
#if 0//ndef QCC74x_USE_HAL_DRIVER
...
void msp_DMA_Disable(DMA_ID_Type dmaId)
{
    ...
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_TOP_CONFIG);
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, DMA_E);
    QCC74x_WR_REG(DMAChs, DMA_TOP_CONFIG, tmpVal);
}
...
void msp_DMA_Channel_Init(DMA_ID_Type dmaId, DMA_Channel_Cfg_Type *chCfg)
{
    ...
    QCC74x_WR_REG(DMAChs, DMA_SRCADDR, chCfg->srcDmaAddr);
    QCC74x_WR_REG(DMAChs, DMA_DSTADDR, chCfg->destDmaAddr);

    tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONTROL);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_TRANSFERSIZE, chCfg->transfLength);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_SWIDTH, chCfg->srcTransfWidth);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DWIDTH, chCfg->dstTransfWidth);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_SBSIZE, chCfg->srcBurstSize);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DBSIZE, chCfg->dstBurstSize);

    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DST_ADD_MODE, chCfg->dstAddMode);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DST_MIN_MODE, chCfg->dstMinMode);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_FIX_CNT, chCfg->fixCnt);

    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_SI, chCfg->srcAddrInc);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DI, chCfg->destAddrInc);
    QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);

    tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONFIG);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_FLOWCNTRL, chCfg->dir);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DSTPERIPHERAL, chCfg->dstPeriph);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_SRCPERIPHERAL, chCfg->srcPeriph);
    QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
}
...
void msp_DMA_Channel_Update_SrcMemcfg(DMA_ID_Type dmaId, uint8_t ch, uint32_t memAddr, uint32_t len)
{
    ...
    /* config channel config*/
    QCC74x_WR_REG(DMAChs, DMA_SRCADDR, memAddr);
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONTROL);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_TRANSFERSIZE, len);
    QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);
}
...
void msp_DMA_Channel_Update_DstMemcfg(DMA_ID_Type dmaId, uint8_t ch, uint32_t memAddr, uint32_t len)
{
    ...
    /* config channel config*/
    QCC74x_WR_REG(DMAChs, DMA_DSTADDR, memAddr);
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONTROL);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_TRANSFERSIZE, len);
    QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);
}
...
uint32_t msp_DMA_Channel_TranferSize(DMA_ID_Type dmaId, uint8_t ch)
{
    ...
    return QCC74x_GET_REG_BITS_VAL(QCC74x_RD_REG(DMAChs, DMA_CONTROL), DMA_TRANSFERSIZE);
}
...
QCC74x_Sts_Type msp_DMA_Channel_Is_Busy(DMA_ID_Type dmaId, uint8_t ch)
{
    ...
    return QCC74x_IS_REG_BIT_SET(QCC74x_RD_REG(DMAChs, DMA_CONFIG), DMA_E) == 1 ? SET : RESET;
}
...
void msp_DMA_Request_Enable(DMA_ID_Type dmaId, uint8_t ch)
{
    ...
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_C0CONFIG);
    printf("DMA_Request_Enable tmpVal = 0x%08lx\r\n", tmpVal);

    tmpVal = QCC74x_SET_REG_BIT(tmpVal, DMA_H);
    printf("DMA_Request_Enable tmpVal = 0x%08lx\r\n", tmpVal);
    QCC74x_WR_REG(DMAChs, DMA_C0CONFIG, tmpVal);
#else
    uint32_t val;
...
uint32_t msp_DMA_SrcAddr_Get(DMA_ID_Type dmaId, uint8_t ch)
{
    ...
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_C0SRCADDR);
...
void msp_DMA_LLI_Init(DMA_ID_Type dmaId, uint8_t ch, DMA_LLI_Cfg_Type *lliCfg)
{
    ...
    tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONFIG);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_FLOWCNTRL, lliCfg->dir);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_DSTPERIPHERAL, lliCfg->dstPeriph);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, DMA_SRCPERIPHERAL, lliCfg->srcPeriph);
    QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
}
...
void msp_DMA_LLI_Update(DMA_ID_Type dmaId, uint8_t ch, uint32_t LLI)
{
    ...
    /* Config channel config */
    //QCC74x_WR_REG(DMAChs, DMA_LLI, LLI);
...
uint32_t msp_DMA_LLI_Get_Counter(DMA_ID_Type dmaId, uint8_t ch)
{
    ...
    return QCC74x_GET_REG_BITS_VAL(QCC74x_RD_REG(DMAChs, DMA_CONFIG), DMA_LLICOUNTER);
}
#if 0
/****************************************************************************/ /**
 ...
 *
*******************************************************************************/
QCC74x_Err_Type DMA_LLI_PpStruct_Set_Transfer_Len(DMA_LLI_PP_Struct *dmaPpStruct, uint16_t Ping_Transfer_len, uint16_t Pong_Transfer_len)
{
    ...
    switch (intType) {
        case DMA_INT_TCOMPLETED:
            if (intMask == UNMASK) {
                ...
                tmpVal = QCC74x_CLR_REG_BIT(QCC74x_RD_REG(DMAChs, DMA_CONFIG), DMA_ITC);
                QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
                tmpVal = QCC74x_SET_REG_BIT(QCC74x_RD_REG(DMAChs, DMA_CONTROL), DMA_I);
                QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);
            } else {
                /* MASK(Disable) this interrupt */
                tmpVal = QCC74x_SET_REG_BIT(QCC74x_RD_REG(DMAChs, DMA_CONFIG), DMA_ITC);
                QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
                tmpVal = QCC74x_CLR_REG_BIT(QCC74x_RD_REG(DMAChs, DMA_CONTROL), DMA_I);
                QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);
            }
        ...
        case DMA_INT_ERR:
            if (intMask == UNMASK) {
                /* UNMASK(Enable) this interrupt */
                tmpVal = QCC74x_CLR_REG_BIT(QCC74x_RD_REG(DMAChs, DMA_CONFIG), DMA_IE);
                QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
            } else {
                /* MASK(Disable) this interrupt */
                tmpVal = QCC74x_SET_REG_BIT(QCC74x_RD_REG(DMAChs, DMA_CONFIG), DMA_IE);
                QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
            }
        ...
        case DMA_INT_ALL:
            if (intMask == UNMASK) {
                /* UNMASK(Enable) this interrupt */
                tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONFIG);
                tmpVal = QCC74x_CLR_REG_BIT(tmpVal, DMA_ITC);
                tmpVal = QCC74x_CLR_REG_BIT(tmpVal, DMA_IE);
                QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
                tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONTROL);
                tmpVal = QCC74x_SET_REG_BIT(tmpVal, DMA_I);
                QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);
            } else {
                /* MASK(Disable) this interrupt */
                tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONFIG);
                tmpVal = QCC74x_SET_REG_BIT(tmpVal, DMA_ITC);
                tmpVal = QCC74x_SET_REG_BIT(tmpVal, DMA_IE);
                QCC74x_WR_REG(DMAChs, DMA_CONFIG, tmpVal);
                tmpVal = QCC74x_RD_REG(DMAChs, DMA_CONTROL);
                tmpVal = QCC74x_CLR_REG_BIT(tmpVal, DMA_I);
                QCC74x_WR_REG(DMAChs, DMA_CONTROL, tmpVal);
            }
 ...
 *
*******************************************************************************/
QCC74x_Err_Type DMA_LLI_PpStruct_Init(DMA_LLI_PP_Struct *dmaPpStruct)
{
    //setup lliList
