...
void xcodec_rx_user_cb(aui_ch_t *context, audio_codec_event_t event, void *arg)
{
    ...
    if (event == QCC74x_EVENT_NODE_READ_COMPLETE) {
        r_node_num++;
    ...
    } else if (event == QCC74x_EVENT_READ_BUFFER_FULL) {
        if (ch->callback) {
...
void xcodec_tx_user_cb(auo_ch_t *context, audio_codec_event_t event, void *arg)
{
    ...
    if (event == QCC74x_EVENT_NODE_WRITE_COMPLETE) {
        w_node_num++;
    ...
    } else if (event == QCC74x_EVENT_WRITE_BUFFER_EMPTY) {
        if (ch->callback) {
