...
int qcc74x_boot2_update_ptable(qcc74x_partition_config_t *ptEntry_hal)
{
    int ret;
    //FIXME force covert
...
int qcc74x_boot2_partition_bus_addr_active(const char *name, uint32_t *addr, uint32_t *size)
{
    ...
    if ((ret = qcc74x_boot2_partition_bus_addr(name, &addr0, &addr1, &size0, &size1, &active))) {
        return ret;
    }
...
int qcc74x_boot2_partition_addr_active(const char *name, uint32_t *addr, uint32_t *size)
{
    ...
    if ((ret = qcc74x_boot2_partition_addr(name, &addr0, &addr1, &size0, &size1, &active))) {
        return ret;
    }
    *addr = active ? addr1 : addr0;
    *size = active ? size1 : size0;
...
int qcc74x_boot2_get_active_entries(int type, qcc74x_partition_config_t *ptEntry_hal)
{
    pt_table_entry_config *ptEntry = (pt_table_entry_config*)ptEntry_hal;
...
void qcc74x_update_mfg_ptable(void)
{
    ...
    printf("====================\r\n");
    if (0 == qcc74x_boot2_get_active_entries_byname((uint8_t *)PT_OTA_TYPE_NAME, (qcc74x_partition_config_t *)(&ptEntry_fw))) {       // ota
        if (0 == qcc74x_boot2_get_active_entries_byname((uint8_t *)PT_MEDIA_TYPE_NAME, (qcc74x_partition_config_t *)(&ptEntry_media))) { // media
