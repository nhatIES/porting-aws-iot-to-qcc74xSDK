...
lfs_t *lfs_xip_init(struct lfs_context *lfs_xip_ctx, struct lfs_config *cfg)
{
    ...
    ret = qcc74x_mtd_open(lfs_xip_ctx->partition_name, &handle, QCC74x_MTD_OPEN_FLAG_BUSADDR);
    if (ret < 0) {
...
{
    struct lfs_context *ctx = c->context;
    return qcc74x_flash_read(ctx->flash_addr + block * c->block_size + off,
                           (uint8_t *)buffer, size);
}
...
{
    struct lfs_context *ctx = c->context;
    return qcc74x_flash_write(ctx->flash_addr + block * c->block_size + off,
                            (uint8_t *)buffer, size);
}
...
{
    struct lfs_context *ctx = c->context;
    return qcc74x_flash_erase(ctx->flash_addr + block * c->block_size, c->block_size);
}
